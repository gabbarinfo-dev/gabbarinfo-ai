...
import { fetchCuratedStoreProducts } from "../../../lib/ecommerceMerchandising";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!verifiedMetaAssets) {
      ...
      const token = meta.fb_user_access_token;
    ...
    if (lockedCampaignState && lockedCampaignState.plan && mode === "meta_ads_plan") {
      ...
      if (lockedCampaignState.stage === "PLAN_PROPOSED") {
        ...
        if (isConfirm) {
          ...
          const genderLabel = nextState.target_gender === "women" ? "Women only" : nextState.target_gender === "men" ? "Men only" : "All genders";
          const offerLine = nextState.offer ? `\n• **Special Offer**: "${nextState.offer}"` : "";
    ...
    if (
      ...
      if (imageUrlMatch) {
        ...
        // ✅ User is skipping — proceed to READY_TO_LAUNCH (AI visual will be created when launched)
        console.log("🖼️ [User Image] User skipped. Image will be generated by Railway worker on launch.");
        ...
        return res.status(200).json({
          ...
          text: `✅ **Campaign Strategy Locked & Ready!**\n\n` +
            ...
            `• **Ad Creative**: High-converting AI visual with your branding & headline (rendered on publish)\n\n` +
            `👉 Reply **LAUNCH** or **YES** to generate your ad creative and publish this campaign live to your Meta Ad Account!`
        });
      } else {
...

{
  ...
  "targeting": {
    "universal_locations": ["${currentLocation}"],
    "age_min": ${lockedCampaignState?.target_age_min || 18},
    "age_max": ${lockedCampaignState?.target_age_max || 65},
    "genders": "${lockedCampaignState?.target_gender || 'all'}",
...
- Remind users that they can connect their WordPress website directly via the [SEO & Web Content Suite](/seo) or Dashboard to enable 1-click publishing, autonomous scheduled blogging cadence, and dual visual generation.
`;
    } else {
      modeFocus = `
...
`;
    ...
    if (!isPlanProposed && mode === "meta_ads_plan" && lockedCampaignState?.service && !lockedCampaignState?.location && !lockedCampaignState?.location_confirmed) {
      ...
      if (input.length >= 2 && !isPureNumber) {
        ...
        // Smart gender suggestion based on service name
        const svc = (lockedCampaignState.service || "").toLowerCase();
        let genderHint = "";
        if (/salon|beauty|bridal|women|ladies|mehndi|makeup|parlour|parlor/.test(svc)) {
          genderHint = " (Based on your service, I recommend **Women only**)";
        } else if (/barber|men'?s|groom/.test(svc)) {
          genderHint = " (Based on your service, I recommend **Men only**)";
        }
        ...
        return res.status(200).json({
          ...
          mode,
          gated: true,
          text: `Target ${locationText} set: **${cleanedLocation}**.\n\nWho should see this ad?${genderHint}\n\n1. Women only\n2. Men only\n3. All genders (recommended for most businesses)`
        });
      } else {
        return res.status(200).json({
          ok: true,
          mode,
          gated: true,
          text: "What is the **Target Location**? You can enter:\n- One or more cities: Mumbai, Ahmedabad\n- States or regions: Texas, Maharashtra\n- Countries: United States, India\n\nSeparate multiple locations with commas."
        });
      }
    }
    ...
    if (!isPlanProposed && mode === "meta_ads_plan" && lockedCampaignState?.location_confirmed && !lockedCampaignState?.target_gender) {
      ...
      if (selectedGender) {
        ...
        const genderLabel = selectedGender === "women" ? "Women only" : selectedGender === "men" ? "Men only" : "All genders";

        return res.status(200).json({
          ok: true,
          mode,
          gated: true,
          text: `Gender targeting: **${genderLabel}**.\n\nWhat age group should we target?\n\n1. Standard (18-65) — recommended for broad reach\n2. Custom — I'll specify my own range`
        });
      } else {
        // Smart suggestion based on service
        const svc = (lockedCampaignState.service || "").toLowerCase();
        let genderHint = "";
        if (/salon|beauty|bridal|women|ladies|mehndi|makeup|parlour|parlor/.test(svc)) {
          genderHint = " (Based on your service, I recommend **Women only**)";
        ...
        return res.status(200).json({
          ...
          mode,
          gated: true,
          text: `Who should see this ad?${genderHint}\n\n1. Women only\n2. Men only\n3. All genders (recommended for most businesses)`
        });
      }
    }
    ...
    if (!isPlanProposed && mode === "meta_ads_plan" && lockedCampaignState?.target_gender && !lockedCampaignState?.age_confirmed) {
      ...
      if (
        ...
        return res.status(200).json({
          ...
          mode,
          gated: true,
          text: "Enter your custom age range (e.g., **25-45**):"
        });
      }
      ...
      return res.status(200).json({
        ...
        mode,
        gated: true,
        text: "What age group should we target?\n\n1. Standard (18-65) — recommended for broad reach\n2. Custom — I'll specify my own range (e.g., 25-45)"
      });
    }
    // Step 6: Budget Confirmation
    if (!isPlanProposed && mode === "meta_ads_plan" && lockedCampaignState?.location && lockedCampaignState?.age_confirmed && !lockedCampaignState?.budget_per_day) {
...
7b. Gender Targeting (Women/Men/All) -> [LOCKED]
7c. Age Range (Standard 18-65 or Custom) -> [LOCKED]
...
`.trim();
    ...
    // 🖼️ RESUME PATH: Stage is READY_TO_LAUNCH, IMAGE_GENERATED, or PLAN_CONFIRMED + user wants to launch
    // This fires on LAUNCH turn (Railway worker handles visual creation and publish)
    const isResumeExecution =
      (lockedCampaignState?.stage === "IMAGE_GENERATED" || lockedCampaignState?.stage === "READY_TO_LAUNCH" || lockedCampaignState?.stage === "PLAN_CONFIRMED") &&
      !!lockedCampaignState?.plan &&
      wantsToLaunch;
    ...
    if (
      ...
      if (stage === "PLAN_CONFIRMED") {
        console.log("🚀 Advancing PLAN_CONFIRMED to READY_TO_LAUNCH without blocking on serverless visual generation");
        state = {
          ...state,
          stage: "READY_TO_LAUNCH",
          user_provided_image_url: state.user_provided_image_url || null,
        };
        currentState = state;
        ...
        if (!wantsToLaunchNow) {
          ...
          return res.status(200).json({
            ...
            mode,
            text: `✅ **Campaign Strategy Locked & Ready!**\n\n` +
              `• **Service / Product**: ${state.service || "Promotional Ad"}\n` +
      ...
      if (!errorOcurred) {
        // ✅ Ready when: stage is READY_TO_LAUNCH, IMAGE_GENERATED, or PLAN_CONFIRMED
        const isReady =
          state.stage === "READY_TO_LAUNCH" ||
          state.stage === "IMAGE_GENERATED" ||
          state.stage === "PLAN_CONFIRMED";
        ...
        if (isReady && wantsLaunch) {
          ...
          try {
            ...
            const finalPayload = {
              ...
              targeting: {
                ...plan.targeting,
                universal_locations: currentState.location
                  ? currentState.location.split(',').map(l => l.trim())
                  : plan.targeting?.universal_locations || [],
                age_min: currentState.target_age_min || plan.targeting?.age_min || 18,
                age_max: currentState.target_age_max || plan.targeting?.age_max || 65,
            ...
            try {
              ...
              const railwayRes = await dispatchMetaCampaignToRailway({
                  ...
                  userProvidedImageUrl: state.user_provided_image_url || state.creative?.userProvidedImageUrl || null,
    ...
    if (effectiveBusinessId && !isNewMetaCampaignRequest && !lowerInstruction.includes("yes")) {
      ...
      if (jsonString) {
        ...
        try {
          ...
          // 🔧 FIX: Convert location_name into universal_locations format
          if (planJson.targeting?.geo_locations?.location_name) {
            const cityName = planJson.targeting.geo_locations.location_name;
            planJson.targeting.universal_locations = [cityName];
            delete planJson.targeting.geo_locations;
          }
          ...
          if (planJson.campaign_data) {
            ...
            planJson = {
              ...
              targeting: {
                universal_locations: t.locations || (lockedCampaignState?.location ? lockedCampaignState.location.split(',').map(l => l.trim()) : []),
                age_min: parseInt(t.age_range?.split("-")[0]) || lockedCampaignState?.target_age_min || 18,
                age_max: parseInt(t.age_range?.split("-")[1]) || lockedCampaignState?.target_age_max || 65,
                genders: lockedCampaignState?.target_gender || "all",
          ...
          if (planJson.campaign_details) {
            ...
            planJson = {
              ...
              targeting: {
                universal_locations: lockedCampaignState?.location
                  ? lockedCampaignState.location.split(',').map(l => l.trim())
                  : [],
                age_min: d.targeting?.age_min || lockedCampaignState?.target_age_min || 18,
                age_max: d.targeting?.age_max || lockedCampaignState?.target_age_max || 65,
                genders: lockedCampaignState?.target_gender || "all"
              },
              ad_sets: [
                {
          ...
          if (planJson.campaign && planJson.adSets && Array.isArray(planJson.adSets)) {
            ...
            // Map Targeting (universal_locations from locked state)
            ...
            planJson = {
              ...
              targeting: {
                universal_locations: lockedCampaignState?.location
                  ? lockedCampaignState.location.split(',').map(l => l.trim())
                  : [],
                age_min: adSet.targeting?.age_min || lockedCampaignState?.target_age_min || 18,
                age_max: adSet.targeting?.age_max || lockedCampaignState?.target_age_max || 65,
                genders: lockedCampaignState?.target_gender || "all"
              },
              ad_sets: [
                {
      ...
      if (looksLikePlan) {
        ...
        const minimalPlan = {
          ...
          targeting: {
            universal_locations: lockedCampaignState?.location
              ? lockedCampaignState.location.split(',').map(l => l.trim())
              : extractedLocation.includes(",") ? extractedLocation.split(",").map(c => c.trim()) : [extractedLocation],
            age_min: lockedCampaignState?.target_age_min || 18,
            age_max: lockedCampaignState?.target_age_max || 65,
            genders: lockedCampaignState?.target_gender || "all"
          },
  ...
  try {
    ...
    if (gAdsState?.stage === "PLAN_PROPOSED" && isConfirm && gAdsState.plan && !justSelectedAccountId) {
      try {
        ...
        const campaignType = plan.campaignType || plan.campaign?.campaignType || gAdsState.intake?.campaign_type || "SEARCH";
        const merchantId = plan.merchantId || plan.campaign?.merchantId || gAdsState.intake?.merchant_id || null;
        const networkSettings = plan.networkSettings || plan.campaign?.networkSettings || gAdsState.intake?.networkSettings || null;
        ...
        const createRes = await createFullGoogleAdsCampaign({
          ...
          loginCustomerId: targetManagerId,
          services,
          curatedProductItemIds: plan.campaign?.curatedProductItemIds || plan.curatedProductItemIds || [],
        });
        ...
        if (networkSettings) {
          const netLabel = networkSettings.targetContentNetwork
            ? "Google Search + Search Partners + Display Network"
            : (networkSettings.targetSearchNetwork ? "Google Search + Search Partners" : "Google Search Only (High Intent)");
          assetLines.push(`• **Network Placements:** 🌐 ${netLabel}`);
        }
...
`;
    ...
    const productUrlMatches = instruction.match(/https?:\/\/[^\s>)]+?\/products\/[a-z0-9-_]+/gi);
    if (productUrlMatches && productUrlMatches.length > 0) {
      intakeData.product_urls = [...new Set(productUrlMatches)];
    }
    ...
    const mergedIntake = {
      ...
      ...(Array.isArray(intakeData.product_urls) && intakeData.product_urls.length > 0 ? { product_urls: intakeData.product_urls } : {}),
    };
    ...
    if (justSelectedAccountId || justSelectedCampaignType || isMissingKeyInfo) {
      ...
      if (activeCampaignType === "PERFORMANCE_MAX") {
        ...
        const isPMaxRetail = activeCampaignType === "PERFORMANCE_MAX_SHOPPING";
        const hasGmc = Boolean(mergedIntake.merchant_id || linkedGmc?.merchantId);

        // STRICT REQUIREMENT: If Google Merchant Center is missing, be 100% upfront in first statement
        // NEVER downgrade or convert to a search ad!
        if (!hasGmc) {
          const noGmcGuidance =
            `${accountNoticeHeader}\n\n` +
            `⚠️ **Google Merchant Center (GMC) Required for Visual Shopping Cards**\n\n` +
            `You selected **${isPMaxRetail ? "Performance Max Shopping (Retail)" : "Standard Shopping"}**, which specifically displays **visual Product Cards (photo, price, title, store name)** in Google Shopping.\n\n` +
            `🛑 **Current Status:** No linked Google Merchant Center catalog was detected for Google Ads account **${activeAccountObj.descriptiveName}** (\`${formattedAccId}\`). Google strictly requires a Merchant Center feed to render visual product cards.\n\n` +
            `**Here is how we can resolve this:**\n` +
            `1. **If you have a Shopify store:** Open your Shopify Admin ➔ **Google & YouTube app** ➔ Confirm your Merchant Center account is linked to Google Ads \`${formattedAccId}\`.\n` +
            `2. **If you already have a GMC account:** Simply reply with your **10-digit Merchant Center ID** (e.g. \`551-389-5166\`), and I will link it.\n` +
            `3. **If you don't have a GMC yet:** Create a free catalog at [merchants.google.com](https://merchants.google.com) and link your store.\n\n` +
            `ℹ️ *Note: We will not downgrade you to a text search ad because visual product cards are a must. Once your GMC is connected or you provide your ID, reply here to launch your shopping cards immediately!*`;
        ...
        const autoSyncNotice = (mergedIntake.merchant_id || connectedShopify || mergedIntake.youtube_video_url)
          ? `\n\n✅ **Auto-Detected Connected Store, Catalog & Media:**\n` +
            ...
            (isPMaxRetail && mergedIntake.youtube_video_url ? `• 🎥 **Paired YouTube Video:** "${mergedIntake.youtube_video_title}" (${mergedIntake.youtube_video_url})\n` : "")
          : "";

        formatIntro = isPMaxRetail
          ? `${accountNoticeHeader}\n\n🛍️⚡ **Performance Max Shopping (Retail) Campaign**\nPMax Shopping displays your visual Google Shopping product cards (Photo + Price + Title + Store Name) across Google Shopping, Search, YouTube, and Display.${autoSyncNotice}\nTo finalize your campaign blueprint, please confirm or adjust:`
          : `${accountNoticeHeader}\n\n🛍️ **Standard Shopping Campaign**\nStandard Shopping showcases your visual product cards (photo, price, store name) directly in Google Shopping and Search product grids.${autoSyncNotice}\nTo finalize your campaign blueprint, please confirm or adjust:`;

        missingList.push(
          "1. 🎯 **What products would you like to feature in this campaign?**\n" +
          "   • **A Specific Category** (e.g., *\"Kundan Jewellery\"*, *\"Sneakers\"*, *\"Watches\"*) — our AI will auto-curate your **Highest-Value Showpieces** + **Highest-Discount Deal Magnets**!\n" +
          "   • **Specific Products** (Paste 2 to 5 product links from your store!)\n" +
          "   • **Entire Catalog** (All products in your store)"
        );

        if (!hasLocation) {
          missingList.push(`2. 📍 **Target Location:** ${mergedIntake.location || "United Kingdom"} *(Default: entire country. Want to target specific cities like London, Manchester, or a different region? Just type it!)*`);
        }
        if (!hasBudget) {
          missingList.push(`3. 💰 **Daily Budget:** What is your target daily budget (e.g. £5/day or ₹1,000/day in ${accountCurrency})?`);
        }
        if (!mergedIntake.bidding_strategy) {
          missingList.push(
            isPMaxRetail
              ? "4. 📈 **Bidding Strategy:** Do you prefer **Maximize Conversions** *(highest sales volume)* or **Maximize Conversion Value** *(highest total store revenue)*?"
              : "4. 📈 **Bidding Strategy:** Do you prefer **Maximize Clicks** *(drive maximum visitors to product pages)* or **Manual CPC**?"
          );
        }
      ...
      if (hasLandingPage) {
        capturedList.push(`• **Landing Page:** ${mergedIntake.landing_page_url}`);
...
${JSON.stringify(candidateSitelinks, null, 2)}
   ...
   - description2: STRICTLY maximum 35 characters`
    ...
    let curatedStoreMerchandising = null;
    if (chosenCampaignType === "PERFORMANCE_MAX_SHOPPING" || chosenCampaignType === "SHOPPING") {
      try {
        curatedStoreMerchandising = await fetchCuratedStoreProducts({
          shopifyConnection: connectedShopify,
          category: mergedIntake.services || null,
          productUrls: mergedIntake.product_urls || [],
          targetCountry: (countryIso || mergedIntake.location || "GB").slice(0, 2).toUpperCase(),
        });
      } catch (curateErr) {
        console.warn("fetchCuratedStoreProducts non-fatal error:", curateErr.message);
      }
    }
...
4. Negative Keywords: Apply the finalized negative keywords (${finalizedNegatives.length} exclusions) to filter out irrelevant product search queries.`;
      ...
      formatJsonAdGroups = `"adGroups": [
    {
      ...
      let shoppingCuratedSummary = "";
      if (curatedStoreMerchandising?.curatedProducts?.length > 0) {
        shoppingCuratedSummary =
          `\n- 🛍️ **Curated Products Targeted:** (${curatedStoreMerchandising.curatedProducts.length} items targeted in Shopping Grid)\n` +
          (curatedStoreMerchandising.showpieces?.length > 0
            ? `  • 💎 **Top Showpieces:** ${curatedStoreMerchandising.showpieces.map((p) => `${p.title} (£${p.price})`).join(", ")}\n`
            : "") +
          (curatedStoreMerchandising.dealMagnets?.length > 0
            ? `  • 🔥 **Top Deals:** ${curatedStoreMerchandising.dealMagnets.map((p) => `${p.title} (${p.discountPercent}% OFF)`).join(", ")}\n`
            : "");
      }
...
- 🌐 **Destination Store:** ${landingUrl}${shoppingCuratedSummary}
  ...
  "adGroups": [
    {
      ...
      let pmaxCuratedSummary = "";
      if (chosenCampaignType === "PERFORMANCE_MAX_SHOPPING" && curatedStoreMerchandising?.curatedProducts?.length > 0) {
        pmaxCuratedSummary =
          `\n- 🛍️ **Curated Product Portfolio (Featured in Google Shopping Cards):**\n` +
          (curatedStoreMerchandising.showpieces?.length > 0
            ? `  • 💎 **High-Value Luxury Showpieces:** ${curatedStoreMerchandising.showpieces.map((p) => `${p.title} (£${p.price})`).join(", ")}\n`
            : "") +
          (curatedStoreMerchandising.dealMagnets?.length > 0
            ? `  • 🔥 **High-Discount Deal Magnets:** ${curatedStoreMerchandising.dealMagnets.map((p) => `${p.title} (${p.discountPercent}% OFF)`).join(", ")}\n`
            : "") +
...
- 📍 **Targeting & Location:** ${targetLocation}
- 💰 **Daily Budget:** ${accountCurrency === "INR" ? "₹" : accountCurrency + " "}${mergedIntake.daily_budget}/day
- 🌐 **Landing Page:** ${landingUrl}${pmaxCuratedSummary}
      ...
      try {
        ...
        if (extractedPlan) {
          ...
          if (extractedPlan) {
            ...
            if (!extractedPlan.campaignType) extractedPlan.campaignType = chosenCampaignType;
            if (!extractedPlan.networkSettings) extractedPlan.networkSettings = resolvedNetworkSettings;
            if (!extractedPlan.merchantId && mergedIntake.merchant_id) extractedPlan.merchantId = mergedIntake.merchant_id;
            if (extractedPlan.campaign) {
              if (!extractedPlan.campaign.campaignType) extractedPlan.campaign.campaignType = chosenCampaignType;
              if (!extractedPlan.campaign.networkSettings) extractedPlan.campaign.networkSettings = resolvedNetworkSettings;
              if (!extractedPlan.campaign.merchantId && mergedIntake.merchant_id) extractedPlan.campaign.merchantId = mergedIntake.merchant_id;
              if (curatedStoreMerchandising?.itemIds?.length > 0) {
                extractedPlan.campaign.curatedProductItemIds = curatedStoreMerchandising.itemIds;
              }
              if (curatedStoreMerchandising?.productImages?.length > 0) {
                extractedPlan.campaign.curatedProductImages = curatedStoreMerchandising.productImages;
              }
            }
            if (curatedStoreMerchandising?.itemIds?.length > 0) {
              extractedPlan.curatedProductItemIds = curatedStoreMerchandising.itemIds;
            }
            if (curatedStoreMerchandising?.productImages?.length > 0) {
              extractedPlan.curatedProductImages = curatedStoreMerchandising.productImages;
            }
          }
        }
      } catch (geminiErr) {
