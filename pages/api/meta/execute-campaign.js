...
function mapObjectiveToODAX(obj) { 
  ...
  if (o === "OUTCOME_TRAFFIC" || o === "TRAFFIC") return 
"OUTCOME_TRAFFIC"; 
...
campaign_name required" }); 
  ...
  const { data: meta, error } = await supabase 
    ...
    .select("fb_ad_account_id, fb_page_id, ig_business_id, 
instagram_actor_id, business_website") 
    .eq("email", clientEmail) 
    .single(); 
 
  if (error || !meta) { 
    return res.status(400).json({ ok: false, message: "Meta connection 
not found" }); 
  } 
...
|| payload.conversion_location === "WEBSITE"; 
      ...
      if (requiresDestinationUrl) { 
        ...
        if (!creative.destination_url) { 
          throw new Error("Please provide a website or landing page for 
  ...
  switch (objective) { 
    ...
    case "OUTCOME_SALES": 
      ...
      if (!adSet.promoted_object || !adSet.promoted_object.pixel_id) { 
        throw new Error("OUTCOME_SALES requires a Pixel ID 
...
!adSet.promoted_object.application_id) { 
        throw new Error("OUTCOME_APP_PROMOTION requires an Application 
ID."); 
      } 
...
instagramActorId, accessToken, forcePhoto = false, placements = []) { 
  ...
  if (!creative || !creative.image_hash) throw new Error("Image Hash is 
