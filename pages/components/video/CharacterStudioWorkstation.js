            ...
            if (jobData.ok) {
              ...
              if (jobData.status === "completed" && jobData.videoUrl) {
                ...
                setGeneratedStory({
                  ...
                  scenes: (jobData.metadata?.scenes && jobData.metadata.scenes.length > 0)
                    ? jobData.metadata.scenes.map((s, idx) => ({
                        sceneNumber: s.sceneNumber || idx + 1,
                        chapter: s.chapter || "",
                        text: (s.spokenAudio || s.text || s.narration || "").replace(/^["']|["']$/g, ""),
                        speaker: s.speaker || "Character",
            ...
            >
              {generatedStory ? (
                <div style={{ flex: 1, position: "relative", overflow: "hidden", background: "#090d16" }}>
                  ...
                  {/* Fallback Preview Overlays (Only shown when client canvas preview is used without master video) */}
                  {!generatedStory.masterVideoUrl && (
                    <>
                      {/* Gradient Overlay */}
                      ...
                      {/* Subtitle Box */}
                      {generatedStory.scenes[activeSceneIndex]?.text && (
