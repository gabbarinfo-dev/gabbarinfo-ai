...
`.trim();
  ...
  async function handleRunAgent() {
    ...
    try {
      ...
      // 🚂 ASYNC META CAMPAIGN JOB POLLING (Zero Vercel Timeout!)
      if (data.metaJobId) {
        const jobId = data.metaJobId;
        const initialText = decodeUnicodeEscapes(data.text || "Publishing campaign to Meta Ads...");
        const initialAssistantMsg = {
          role: "assistant",
          text: `GabbarInfo Agent:\n\n${initialText}`,
        };
        ...
        const pollInterval = setInterval(async () => {
          pollCount++;
          try {
            const statusRes = await fetch(`/api/meta/job-status?jobId=${encodeURIComponent(jobId)}`);
            if (statusRes.ok) {
              const statusData = await statusRes.json();
              if (statusData.status === "completed") {
                clearInterval(pollInterval);
                const resObj = statusData.result || {};
                const campaignId = resObj.campaignId || resObj.id || "Live";
                const adSetId = resObj.adSetId || resObj.ad_set_id || "Active";
                const adId = resObj.adId || resObj.ad_id || "Active";
                const creativeUrl = resObj.imageUrl || "";

                const successText = `🎉 **Campaign Live on Meta!**\n\nYour ad campaign has been successfully published directly to your Meta Ad Account.\n\n• **Campaign Name**: ${data.campaignName || "Meta Campaign"}\n• **Campaign ID**: \`${campaignId}\`\n• **Ad Set ID**: \`${adSetId}\`\n• **Ad ID**: \`${adId}\`\n• **Status**: **ACTIVE / IN REVIEW**\n\n${creativeUrl ? `![Ad Creative](${creativeUrl})\n\n` : ""}Your ads will begin delivering to your target audience once Meta's automated ad review completes (usually within 15–60 minutes).`;
                ...
                updateChatWithAssistantMessage(pseudoUserText, updatedMessages, finalAssistantMsg);
                setAgentLoading(false);

                // Top up credits
                if (role !== "owner" && !unlimited) {
                  try {
                    await fetch("/api/credits/consume-campaign-publish", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      credentials: "include",
                ...
                clearInterval(pollInterval);
                setAgentError("Meta campaign creation error: " + (statusData.error || "Unknown worker error"));
                setAgentLoading(false);
                return;
              } else if (statusData.stage) {
                setAgentResponse(`⏳ ${statusData.stage} (${statusData.progress || 50}%)`);
              }
            }
          } catch (pollErr) {
            console.warn("Job status poll error:", pollErr);
          }

          if (pollCount >= 75) { // 75 * 2s = 150s max
            clearInterval(pollInterval);
            setAgentError("Campaign creation is still completing in the background on Railway. Please refresh your Meta Ads Manager in a few moments.");
            setAgentLoading(false);
          }
        }, 2000);
