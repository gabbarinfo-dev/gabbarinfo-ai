...
export async function getAccountHierarchy({ refreshToken }) {
  ...
  for (const topId of topLevelIds) {
    ...
    const details = await getCustomerDetails({
      accessToken,
      customerId: topId,
      // No loginCustomerId here — we're checking the top-level account itself
    });

    if (!details.ok) {
      console.warn(`Could not get details for account ${topId}:`, details.error);
      continue;
    }
    ...
    if (details.isManager) {
      ...
      try {
        const url = `https://googleads.googleapis.com/v18/customers/${topId}/googleAds:search`;
