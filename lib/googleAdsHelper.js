...
/**
 * Get Full Account Hierarchy under a Manager Account (MCC)
 * Uses customer_client GAQL query to retrieve all child client accounts.
 * This is the correct approach for Agency/MCC accounts.
 *
 * Flow:
 *   1. Exchange refresh token → access token
 *   2. Query customer_client table under MCC (GOOGLE_ADS_LOGIN_CUSTOMER_ID)
 *   3. Return all active, non-manager client accounts
 *   4. Fallback to listAccessibleCustomers if MCC query fails
 */
export async function getAccountHierarchy({ refreshToken }) {
  ...
  // ── Strategy 1: Query customer_client under the known MCC ──────────────
  if (managerCustomerId) {
    try {
      const url = `https://googleads.googleapis.com/v18/customers/${managerCustomerId}/googleAds:search`;
      const resp = await fetch(url, {
        method: "POST",
        headers: getHeaders(accessToken, managerCustomerId),
      ...
      if (resp.ok && Array.isArray(json.results)) {
        for (const row of json.results) {
          ...
          // Skip the MCC itself, skip manager accounts, skip cancelled accounts
          if (cid === managerCustomerId) continue;
          if (client.manager) continue;
          if (client.status === "CANCELED" || client.status === "CANCELLED") continue;

          accounts.push({
            customerId: cid,
            descriptiveName: client.descriptiveName || `Google Ads Account (${cid.slice(0, 3)}-${cid.slice(3, 6)}-${cid.slice(6)})`,
        ...
        if (accounts.length > 0) {
          return { ok: true, accessToken, accounts };
        }

        // customer_client query worked but returned empty — no sub-accounts
        return {
          ok: true,
          accessToken,
          accounts: [],
          message: "No active client accounts found under the Manager Account.",
        };
      }

      // Log the error for debugging but fall through to Strategy 2
      console.warn("customer_client hierarchy query failed:", JSON.stringify(json));
    } catch (hierarchyErr) {
      console.warn("Error querying customer_client hierarchy:", hierarchyErr.message);
    }
  }

  // ── Strategy 2: Fallback — listAccessibleCustomers ─────────────────────
  try {
    ...
    const customerIds = resourceNames
      .map((rn) => cleanCustomerId(rn.replace("customers/", "")))
      .filter((cid) => cid !== managerCustomerId); // exclude MCC itself
    ...
    for (const cid of customerIds) {
      ...
      if (details.ok && !details.isManager) {
        accounts.push({
          customerId: cid,
    ...
    return {
      ok: false,
      status: 500,
      json: { error: String(fallbackErr.message || fallbackErr) },
    };
  }
}
