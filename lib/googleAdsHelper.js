...
export async function createCampaignAssetsAndLink({
  ...
  // 1. Prepare Sitelink Asset Operations (NO top-level 'name' field on SitelinkAsset to prevent CANNOT_SET_FIELD)
  if (Array.isArray(sitelinks) && sitelinks.length > 0) {
    sitelinks.slice(0, 6).forEach((s) => {
  ...
  // 2. Prepare Callout Asset Operations (NO top-level 'name' field on CalloutAsset)
  ...
  // 3. Prepare Call Asset Operation (NO top-level 'name' field on CallAsset)
  ...
  // 4. Prepare Business Name Asset Operation (Google Ads Text Asset linked as BUSINESS_NAME)
  if (businessName) {
...
/**
 * Configure Campaign Conversion Goals: Disable MESSAGE_LEAD to avoid message/image warnings,
 * and ensure PHONE_CALL_LEAD and WEBSITE_LEADS are set appropriately.
 */
export async function configureCampaignConversionGoals({
  ...
  try {
    const operations = [
      {
        update: {
          resourceName: `customers/${targetId}/campaignConversionGoals/${campaignId}~MESSAGE_LEAD~WEBSITE`,
          biddable: false,
        },
        updateMask: "biddable",
      },
      {
        update: {
          resourceName: `customers/${targetId}/campaignConversionGoals/${campaignId}~MESSAGE_LEAD~GOOGLE_HOSTED`,
          biddable: false,
        },
        updateMask: "biddable",
      },
    ...
    if (isCallGoal) {
      operations.push(
        {
          update: {
            resourceName: `customers/${targetId}/campaignConversionGoals/${campaignId}~PHONE_CALL_LEAD~GOOGLE_HOSTED`,
            biddable: true,
          },
          updateMask: "biddable",
        },
        {
          update: {
            resourceName: `customers/${targetId}/campaignConversionGoals/${campaignId}~PHONE_CALL_LEAD~WEBSITE`,
            biddable: true,
          },
          updateMask: "biddable",
        }
    ...
    const url = `https://googleads.googleapis.com/v22/customers/${targetId}/campaignConversionGoals:mutate`;
    const resp = await fetch(url, {
      method: "POST",
      headers: getHeaders(accessToken, loginCustomerId),
...
export async function createFullGoogleAdsCampaign({
  ...
  // 5) Create and Link Campaign Assets (Sitelinks, Callouts, Call Assets, Business Name)
  ...
  const actualCallouts = Array.isArray(callouts) && callouts.length > 0
    ? callouts
    : (campaign.callouts || []);
  const actualBusinessName = businessName || campaign.businessName || null;

  let assetResults = { ok: true, linkedAssets: [] };
  if (actualSitelinks.length > 0 || actualCallouts.length > 0 || actualCallAsset || actualBusinessName) {
    try {
