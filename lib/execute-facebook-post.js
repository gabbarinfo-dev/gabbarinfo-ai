import { supabaseServer } from "./supabaseServer.js";

/**
 * Execute a Facebook Page post publishing via Meta Graph API.
 * 
 * @param {Object} params
 * @param {string} params.userEmail - User's email to fetch Meta connection
 * @param {string} params.imageUrl - Publicly accessible URL of the image
 * @param {string} params.caption - The post caption/message
 * @param {string} [params.targetPageId] - Optional specific page ID override
 */
export async function executeFacebookPost({
  userEmail,
  imageUrl,
  caption,
  link = null,
  targetPageId = null,
  skipPermalinkFetch = false,
}) {
  if (!userEmail) throw new Error("userEmail is required for Facebook publishing.");
  if (!link && (!imageUrl || typeof imageUrl !== "string" || !imageUrl.startsWith("http"))) {
    throw new Error("Facebook post must provide either a link URL or a publicly accessible image URL.");
  }

  console.log(`[Facebook Organic] Starting publish for ${userEmail}...`);

  // 1. Fetch Meta Connection data from Supabase
  const cleanEmail = userEmail.toLowerCase().trim();
  const { data: meta, error } = await supabaseServer
    .from("meta_connections")
    .select("fb_page_id, fb_page_access_token, fb_user_access_token")
    .ilike("email", cleanEmail)
    .order("updated_at", { ascending: false })
    .limit(1)
    .maybeSingle();

  if (error) {
    console.error(`[Facebook Organic] Error fetching meta connection:`, error);
  }

  const userToken = meta?.fb_user_access_token || process.env.META_SYSTEM_USER_TOKEN;
  if (!userToken && !meta?.fb_page_access_token) {
    throw new Error("Facebook connection not found. Please connect your Facebook account in the dashboard.");
  }

  // 2. Resolve Page ID
  const pageId = targetPageId || 
    (meta?.fb_page_id ? meta.fb_page_id.split(",")[0].trim() : null) || 
    process.env.FB_PAGE_ID;

  if (!pageId) {
    throw new Error("No Facebook Page ID linked to your account. Please connect a Facebook Page in the dashboard.");
  }

  const API_VERSION = "v21.0";

  // 3. Resolve Page Access Token
  // Priority A: Stored fb_page_access_token
  let pageAccessToken = meta?.fb_page_access_token;

  // Priority B: Fetch from /me/accounts
  if (!pageAccessToken && userToken) {
    try {
      const accountsResp = await fetch(`https://graph.facebook.com/${API_VERSION}/me/accounts?access_token=${encodeURIComponent(userToken)}`);
      const accountsJson = await accountsResp.json();

      if (accountsJson?.data && Array.isArray(accountsJson.data)) {
        const matchedPage = accountsJson.data.find(p => String(p.id) === String(pageId));
        if (matchedPage?.access_token) {
          pageAccessToken = matchedPage.access_token;
          console.log(`[Facebook Organic] Resolved Page Access Token from /me/accounts for page ${matchedPage.name || pageId}`);
        }
      }
    } catch (accErr) {
      console.warn(`[Facebook Organic] /me/accounts query failed:`, accErr.message);
    }
  }

  // Priority C: Direct Page Token lookup via /{page_id}?fields=access_token
  if (!pageAccessToken && userToken) {
    try {
      const pageTokenResp = await fetch(`https://graph.facebook.com/${API_VERSION}/${pageId}?fields=access_token&access_token=${encodeURIComponent(userToken)}`);
      const pageTokenJson = await pageTokenResp.json();
      if (pageTokenJson?.access_token) {
        pageAccessToken = pageTokenJson.access_token;
        console.log(`[Facebook Organic] Resolved Page Access Token from /{page_id}?fields=access_token`);
      }
    } catch (ptErr) {
      console.warn(`[Facebook Organic] Direct page token fetch failed:`, ptErr.message);
    }
  }

  // Fallback to user token if page token could not be resolved
  let tokenToUse = pageAccessToken || userToken;

  let res;
  let json = {};

  /**
   * Internal helper: attempt a single publish pass with the given token.
   * Handles both link-card and photo post paths.
   */
  async function attemptPublish(token) {
    let _res;
    let _json = {};

    // 4A. Link Card post to /{page_id}/feed
    if (link) {
      const feedUrl = `https://graph.facebook.com/${API_VERSION}/${pageId}/feed`;
      console.log(`[Facebook Organic] Publishing Clickable Link Card to Feed URL: ${feedUrl}`);

      const feedParams = new URLSearchParams();
      feedParams.append("link", link);
      feedParams.append("message", caption || "");
      feedParams.append("access_token", token);

      try {
        _res = await fetch(feedUrl, { method: "POST", body: feedParams });
        _json = await _res.json().catch(() => ({}));
        console.log(`[Facebook Organic] Feed Link Card Response:`, _json);
      } catch (feedErr) {
        console.warn(`[Facebook Organic] Feed link post network error:`, feedErr.message);
      }
    }

    // 4B. Photo post (primary or fallback when link post failed)
    if (!_res || !_res.ok) {
      if (imageUrl) {
        const publishUrl = `https://graph.facebook.com/${API_VERSION}/${pageId}/photos`;
        console.log(`[Facebook Organic] ${link ? "Fallback to" : ""} Photo URL: ${publishUrl}`);

        const photoParams = new URLSearchParams();
        photoParams.append("url", imageUrl);
        photoParams.append("message", caption || "");
        photoParams.append("caption", caption || "");
        photoParams.append("access_token", token);

        _res = await fetch(publishUrl, { method: "POST", body: photoParams });
        _json = await _res.json().catch(() => ({}));
        console.log(`[Facebook Organic] Photo Response:`, _json);
      }
    }

    return { res: _res, json: _json };
  }

  // 4. First publish attempt with current token
  ({ res, json } = await attemptPublish(tokenToUse));

  // Auto-recovery for code 190 (invalidated/expired page session)
  // Fixed: recovery now works for both link-card and photo posts (no scoping issues)
  if (res && !res.ok && json.error?.code === 190 && userToken) {
    console.warn("[Facebook Organic] Token invalidated (code 190). Attempting automatic refresh via userToken...");
    try {
      const freshTokenResp = await fetch(`https://graph.facebook.com/${API_VERSION}/${pageId}?fields=access_token&access_token=${encodeURIComponent(userToken)}`);
      const freshTokenJson = await freshTokenResp.json();
      if (freshTokenJson?.access_token) {
        console.log("[Facebook Organic] Fresh Page Token obtained, retrying publish...");
        tokenToUse = freshTokenJson.access_token;
        ({ res, json } = await attemptPublish(tokenToUse));

        if (res?.ok) {
          // Persist the renewed token to Supabase for future posts
          await supabaseServer
            .from("meta_connections")
            .update({ fb_page_access_token: freshTokenJson.access_token, updated_at: new Date().toISOString() })
            .ilike("email", cleanEmail);
          console.log("[Facebook Organic] Renewed page token persisted to Supabase.");
        }
      }
    } catch (refreshErr) {
      console.warn("[Facebook Organic] Token refresh retry failed:", refreshErr.message);
    }
  }

  if (!res || !res.ok) {
    const errorMsg = json?.error?.message || `Meta Page Publish Error (Status: ${res?.status})`;
    console.error(`[Facebook Organic] Publishing Failed:`, json);

    if (json?.error?.code === 200) {
      throw new Error(
        `Permissions Error (#200): Your Facebook connection token lacks 'pages_manage_posts' authorization. Since you just added this permission in your Meta App, please go to the dashboard, click 'Disconnect Facebook', and then 'Connect Facebook' to re-authorize with the new publishing permission.`
      );
    }

    throw new Error(`Facebook Page Publishing Failed: ${errorMsg}`);
  }

  const postId = json.post_id || json.id;
  let postUrl = postId ? `https://www.facebook.com/${postId}` : `https://www.facebook.com/${pageId}`;

  // Fetch the authentic, clickable permalink_url directly from Meta Graph API (optional, skipped during high-speed cron cycles)
  if (postId && tokenToUse && !skipPermalinkFetch) {
    try {
      const pResp = await fetch(`https://graph.facebook.com/${API_VERSION}/${postId}?fields=permalink_url&access_token=${encodeURIComponent(tokenToUse)}`);
      const pJson = await pResp.json();
      if (pJson?.permalink_url) {
        postUrl = pJson.permalink_url;
      }
    } catch (_) {}
  }

  return {
    ok: true,
    pageId,
    photoId: json.id,
    postId,
    postUrl,
    responseJson: json,
  };
}
