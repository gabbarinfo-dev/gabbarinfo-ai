...
export async function executeFacebookPost({
  ...
  // 4A. If link is provided, attempt Link Card post to /{page_id}/feed (Screenshot 3 style)
  if (link) {
    ...
    feedParams.append("access_token", tokenToUse);

    try {
      res = await fetch(feedUrl, { method: "POST", body: feedParams });
      json = await res.json().catch(() => ({}));
      console.log(`[Facebook Organic] Feed Link Card Response:`, json);
    } catch (feedErr) {
      console.warn(`[Facebook Organic] Feed link post network error:`, feedErr.message);
    }
  }

  // 4B. Fallback to Photo post if no link or if feed link card failed
  if (!res || !res.ok) {
    if (imageUrl) {
      const publishUrl = `https://graph.facebook.com/${API_VERSION}/${pageId}/photos`;
      console.log(`[Facebook Organic] Fallback to Photo URL: ${publishUrl}`);
      ...
      json = await res.json().catch(() => ({}));
      console.log(`[Facebook Organic] Photo Response:`, json);
    }
  }

  // Auto-recovery for code 190 (invalidated/expired page session)
  if (!res.ok && json.error?.code === 190 && userToken) {
    ...
    try {
      ...
      if (freshTokenJson?.access_token) {
        ...
        params.set("access_token", freshTokenJson.access_token);
        res = await fetch(publishUrl, { method: "POST", body: params });
        json = await res.json().catch(() => ({}));
