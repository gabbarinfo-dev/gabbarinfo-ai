...
export const ALL_SERVICES = [
  ...
  { key: "VIDEO", label: "AI Cinema Studio" },
  { key: "GMB_AUTOPILOT", label: "Local Maps (GMB)" },
];
...
export function getOrCreateTenantConfig(registry, userEmail) {
  ...
  if (!registry[norm]) {
    ...
    registry[norm] = {
      email: norm,
      features: isOwner ? [...ALL_SERVICE_KEYS] : [],
      isSuspended: false,
      maxBusinesses: isOwner ? 999 : 1,
      subscription: {
        status: isOwner ? "active" : "inactive",
        plan: isOwner ? "unlimited_owner" : "none",
        startDate: new Date().toISOString(),
        expiresAt: isOwner
          ? new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString()
          : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        durationDays: isOwner ? 365 : 0,
      },
...
/**
 * Resolves the default list of entitled switchboard service keys for a plan.
 * Returns empty array if plan is 'none' or null.
 */
export function getFeaturesForPlan(planId) {
  ...
  const plan = getPlanConfig(norm);
  if (!plan) return [];

  const features = [];
  if (plan.features?.SEO) features.push("SEO");
  if (plan.features?.SOCIAL) features.push("SOCIAL");
  if (plan.features?.SOCIAL || plan.features?.SEO || plan.features?.SOCIAL_AUTOPILOT || plan.features?.SEO_AUTOPILOT) {
    features.push("SOCIAL_PLANNER");
  }
  if (plan.features?.META_ADS) features.push("META_ADS");
  if (plan.features?.GOOGLE_ADS) features.push("GOOGLE_ADS");
  if (plan.features?.IMAGE_GENERATION) features.push("IMAGE_GENERATION");
  if (plan.features?.AI_CHAT) features.push("AI_CHAT");
  if (plan.features?.VIDEO) features.push("VIDEO");

  return features;
}

/**
 * Replaces the entire features list for a tenant (e.g. upon plan change or reset).
 */
export async function setTenantFeatures(userEmail, featuresArray) {
  const registry = await getTenantRegistry();
  const config = getOrCreateTenantConfig(registry, userEmail);

  config.features = Array.isArray(featuresArray) ? [...featuresArray] : [];
  config.updatedAt = new Date().toISOString();
