...
export async function generatePlatformGraphic({
  ...
  try {
    ...
    try {
      ...
      const isSeoVisual = actionType === "SEO_HERO" || actionType === "SEO_MID";
      if (!isSeoVisual) {
        console.warn(`[ImageService] OpenAI models failed or timed out, trying visual fallback:`, fallbackErr.message);
        try {
