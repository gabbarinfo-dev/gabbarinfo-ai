...
export async function generatePlatformGraphic({
  ...
  // 3. Approved Model Cascade: gpt-image-2-2026-04-21 (fastest ~10s) -> gpt-image-2 -> gpt-image-1.5
  ...
  // Maximum 22 seconds per model to prevent serverless function timeouts
  const withTimeout = (promise, ms = 22000) =>
    Promise.race([
      promise,
...
/**
 * Cleanup / delete ephemeral image after publishing or when a campaign is reset.
 * @param {string|string[]} storageFiles - Filename(s) or Supabase public URL(s)
 */
export async function cleanupEphemeralImage(storageFiles) {
  if (!storageFiles) return;
  const filesArray = Array.isArray(storageFiles) ? storageFiles : [storageFiles];
  const cleanNames = filesArray
    .filter(Boolean)
    .map((f) => {
      if (typeof f !== "string") return null;
      // If full Supabase URL was passed, extract the filename
      if (f.includes("/instagram-creatives/")) {
        const parts = f.split("/instagram-creatives/");
  ...
  try {
    const { error } = await supabaseServer.storage.from("instagram-creatives").remove(cleanNames);
    if (error) {
      console.warn("[ImageService] Storage cleanup warning:", error.message);
    } else {
      console.log(`[ImageService] Ephemeral storage cleaned up: ${cleanNames.join(", ")}`);
    }
  } catch (err) {
