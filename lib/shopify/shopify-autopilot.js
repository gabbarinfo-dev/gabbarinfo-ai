...
export async function runShopifyAutopilotCycle({ force = false, email = null, logger = console.log } = {}) {
  logger("[Shopify Autopilot] Starting autonomous eCommerce blog generation cycle...");

  let query = supabase.from("agent_memory").select("email, content").eq("memory_type", "shopify_connection");
  if (email) {
  ...
  for (const connItem of connections) {
    ...
    let conn = {};
    try {
      conn = typeof connItem.content === "string" ? JSON.parse(connItem.content) : connItem.content;
    } catch (_) {
      continue;
    }
    ...
    try {
      ...
      // 3. Fetch recent titles
      let existingTitles = [];
      try {
        const aResp = await fetch(
          `https://${shop}/admin/api/2024-01/blogs/${blogId}/articles.json?limit=25&fields=id,title,handle`,
          { headers: { "X-Shopify-Access-Token": accessToken, "Content-Type": "application/json" } }
        );
        if (aResp.ok) {
          const aData = await aResp.json();
          existingTitles = (aData.articles || []).map((a) => a.title);
        }
      } catch (_) {}

      // 4. Topic Selection (Priority: User Topic Lineup Queue)
      const brandName = conn.shopName || "Our Store";
...
1. Catchy H1 title matching user search intent.
2. Engaging introduction with hook.
3. 6 to 8 comprehensive H2/H3 sub-sections.
4. Rich HTML formatting with <ul>, <ol>, <strong>, and <blockquote> styling callouts.
5. Actionable Buyer's Guide & Styling Tips.
6. FAQ section answering 4-5 high-volume search queries.
7. Concluding CTA directing readers to explore ${brandName}'s collections.
...
Format response strictly as JSON:
{
  ...
  "imagePrompt": "Detailed photographic prompt for DALL-E 3 featured image"
}`;
      ...
      // 6. Generate Ultra-HD Hero Image
      logger(`[Shopify Autopilot] Generating ultra-HD hero image via DALL-E 3 for ${shop}...`);
      let imageData = { imageUrl: null, base64: null };
      const imagePrompt = `${articleData.imagePrompt || `Editorial high-end commercial hero image for "${articleData.title}"`}. Hyper-realistic luxury commercial photography, 8k resolution, cinematic lighting, ultra-detailed textures, shot on 85mm lens. No text, no logos.`;
