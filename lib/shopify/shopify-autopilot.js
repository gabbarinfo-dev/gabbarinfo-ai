  ...
  "imagePrompt": "Detailed photographic prompt for featured image"
      ...
      if (!config.isDraft && (config.autoShareFacebook || config.autoShareInstagram)) {
        try {
          ...
          const bKey = (conn.businessName || conn.name || "").toLowerCase().replace(/[^a-z0-9]/g, "");
          ...
          const matchedBrand = brandProfiles.find((b) => {
            ...
            return (bKey && bProfileKey && (bKey === bProfileKey || bProfileKey.includes(bKey) || bKey.includes(bProfileKey))) ||
                   (bUrl && (normDomain.includes(bUrl) || bUrl.includes(normDomain))) ||
          ...
          if (matchedBrand && matchedBrand.pageId && matchedBrand.pageToken) {
            ...
            const shouldShareIg = (config.autoShareInstagram === true || config.autoShareInstagram === undefined) && matchedBrand.igId && matchedBrand.pageToken && publicHeroImg;
            if (shouldShareIg) {
              try {
