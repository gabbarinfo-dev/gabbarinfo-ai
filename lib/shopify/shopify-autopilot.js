...
Format response strictly as JSON:
{
  ...
  "imagePrompt": "Detailed photographic prompt for featured image"
      ...
      if (!config.isDraft && (config.autoShareFacebook || config.autoShareInstagram)) {
        try {
          const { data: brandMems } = await supabase
            .from("agent_memory")
            .select("content")
          ...
          const normDomain = cleanDomain.toLowerCase().replace(/[^a-z0-9]/g, "");
          const normName = String(conn.name || "").toLowerCase().replace(/[^a-z0-9]/g, "");

          const matchedBrand = brandProfiles.find((b) => {
            const bUrl = String(b.websiteUrl || "").toLowerCase().replace(/[^a-z0-9]/g, "");
            ...
            return (bUrl && (normDomain.includes(bUrl) || bUrl.includes(normDomain))) ||
                   (bName && normName && (bName.includes(normName) || normName.includes(bName))) ||
                   (bIg && normName && (bIg.includes(normName) || normName.includes(bIg)));
          });
          ...
          if (matchedBrand && matchedBrand.pageId && matchedBrand.pageToken) {
            ...
            // Share to Instagram if configured
            const publicHeroImg = createdArticle.image?.src;
            if (config.autoShareInstagram && matchedBrand.igId && publicHeroImg) {
              try {
                const igContainerUrl = `https://graph.facebook.com/${API_VERSION}/${matchedBrand.igId}/media`;
                const igCaption = `${shareTitle}\n\n${shareDesc}\n\n🔗 Link in bio to read full guide!\n\n#${(conn.name || "Jewellery").replace(/\s+/g, "")} #TrendingStyles`;
                const cParams = new URLSearchParams();
                cParams.append("image_url", publicHeroImg);
