...
export async function getBusinessSubscriptionState(businessId, userEmail = null) {
  ...
  const isSuperAdmin =
    ...
    normEmail === "shopify-tester@gabbarinfo.com" ||
    Boolean(process.env.OWNER_EMAIL && normEmail === process.env.OWNER_EMAIL.toLowerCase());
  ...
  // 1. Authoritative check: Tenant Registry (Admin Entitlement Engine)
  if (normEmail) {
    try {
      const registry = await getTenantRegistry();
      const tenant = registry[normEmail];
      if (tenant?.subscription && tenant.subscription.plan && tenant.subscription.plan !== "none") {
        const planId = tenant.subscription.plan.toLowerCase().trim();
        const basePlan = getPlanConfig(planId);
        const plan = {
          ...basePlan,
          features: { ...(basePlan?.features || {}) },
        };

        // If admin explicitly configured features on the switchboard, ensure they are enabled
        if (Array.isArray(tenant.features)) {
          tenant.features.forEach((featKey) => {
            plan.features[featKey] = true;
          });
        }

        const now = new Date();
        const cycleStart = tenant.subscription.startDate || new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
        ...
        return {
          ...
          planId,
          plan,
          features: tenant.features || plan.features,
          isUnlimited: false,
          cycleStart,
          cycleEnd,
          nextResetDate: cycleEnd,
      ...
      console.warn("[QuotaService] Tenant registry lookup warning:", regErr.message);
    }
  }
  ...
  try {
    ...
    if (!sub && normEmail) {
      const fallbackBizId = `biz_${normEmail.replace(/[^a-zA-Z0-9]/g, "_")}`;
      const { data, error } = await supabaseServer
        .from("subscriptions")
        .select("plan_id, status, cycle_start, cycle_end, current_period_end")
    ...
    if (sub && sub.plan_id && sub.plan_id !== "none") {
      const planId = sub.plan_id.toLowerCase().trim();
