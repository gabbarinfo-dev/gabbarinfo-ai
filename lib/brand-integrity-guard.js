/**
 * lib/brand-integrity-guard.js
 *
 * UNBREAKABLE MULTI-TENANT & MULTI-STORE BRAND INTEGRITY & ZERO CROSS-CONTAMINATION DEFENSE
 * Shared ES module for Next.js API routes and internal services.
 */

export const KNOWN_INDUSTRY_VOCABULARY = {
  ASTROLOGY_PALMISTRY: [
  ...
  try {
    ...
    for (const m of mems || []) {
      ...
      if (
        ...
        combinedForeignText.includes("necklace") ||
        combinedForeignText.includes("earring") ||
        combinedForeignText.includes("fashion") ||
        combinedForeignText.includes("couture")
      ) {
      ...
      if (
        ...
        combinedForeignText.includes("google ads") ||
        combinedForeignText.includes("meta ads") ||
        combinedForeignText.includes("graphic design") ||
        combinedForeignText.includes("video edit")
      ) {
        KNOWN_INDUSTRY_VOCABULARY.DIGITAL_MARKETING_TECH.forEach((term) => forbiddenTerms.add(term));
      }
    }
  } catch (err) {
    console.warn("[BrandIntegrityGuard] Warning building cross-brand negative list:", err.message);
  }

  return {
    forbiddenTerms: Array.from(forbiddenTerms).filter((t) => t && t.length > 2),
    foreignBrands,
  };
}
 ...
 */
export function checkTopicSimilarityAgainstHistory({
  topic,
  pastTitles = [],
...
function matchesForbiddenTerm(sourceText, term) {
  if (!sourceText || !term) return false;
  const s = normalizeText(sourceText);
  const t = normalizeText(term);
  if (s.includes(t)) return true;
  const tWords = t.split(/\s+/).filter((w) => w.length > 3);
  if (tWords.length > 1) {
    const allMatch = tWords.every((w) => s.includes(w));
 ...
 */
export function validateTopicRelevance({
  topic,
  primaryKeyword = "",
  secondaryKeywords = [],
  forbiddenTerms = [],
 ...
 */
export function getVisualGuardDirectives({ businessName, industry, currentSiteUrl }) {
