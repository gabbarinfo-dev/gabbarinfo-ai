...
export async function loadCreativeState(supabase, email) {
    ...
    try {
        ...
        // FIX: Load by active_creative_session_id primarily
        let activeSessionId = content.active_creative_session_id;

        // FALLBACK: If no active pointer, look for the most recent non-completed session
        if (!activeSessionId && content.creative_sessions) {
            const sessions = Object.entries(content.creative_sessions)
                .filter(([_, s]) => s.stage !== "COMPLETED")
                .sort((a, b) => new Date(b[1].updated_at) - new Date(a[1].updated_at));

            if (sessions.length > 0) {
                activeSessionId = sessions[0][0];
                console.log("🧠 [Memory Fallback] Found active session via search:", activeSessionId);
            }
        }
...
export async function clearCreativeState(supabase, email, sessionId = null) {
    if (!email) return;
    ...
    try {
        ...
        // Remove active pointer if matched or if none provided
        if (!sessionId || content.active_creative_session_id === sessionId) {
