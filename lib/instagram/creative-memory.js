...
export async function loadCreativeState(supabase, email, businessId) {
    ...
    try {
        ...
        // Fix: If no businessId provided, try to find an active session
        if (!businessId && content.business_answers) {
            const keys = Object.keys(content.business_answers);
            for (const key of keys) {
                const s = content.business_answers[key].creative_state;
                // If there's a state that is NOT completed, resume it
                if (s && s.stage !== "COMPLETED") {
                    return s;
                }
            }
        }
