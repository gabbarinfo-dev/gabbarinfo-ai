...
export async function generateImage(state, visualMood, taglineOverride) {
    ...
    // 1. Attempt OpenAI image generation if API key is present
    if (process.env.OPENAI_API_KEY) {
        try {
            ...
            console.warn("[Image Generation] OpenAI image service unavailable (quota or error):", openAiErr.message);
        }
    }

    // 2. High-Quality Commercial Photography Fallback (Pexels) - Zero watermarks, authentic commercial relevance
    if (!imageBuffer && process.env.PEXELS_API_KEY) {
        try {
            console.log(`📸 Searching authentic high-res commercial photography for "${service}" (${industry})...`);
            const query = encodeURIComponent(`${service} ${industry}`.trim());
            const pexelsRes = await fetch(`https://api.pexels.com/v1/search?query=${query}&per_page=5&orientation=square`, {
                headers: { Authorization: process.env.PEXELS_API_KEY }
            });
            if (pexelsRes.ok) {
                const pexelsData = await pexelsRes.json();
                const photo = pexelsData.photos?.[0] || pexelsData.photos?.[1];
                if (photo?.src?.large2x || photo?.src?.large) {
                    ...
                    if (photoFetch.ok) {
                        imageBuffer = Buffer.from(await photoFetch.arrayBuffer());
                        usedModel = "pexels-commercial-photography";
                        console.log(`✅ Retrieved premium commercial photo for "${service}": ${photo.id}`);
                    }
                }
            }
        } catch (pexErr) {
            console.warn("[Image Generation] Pexels fallback warning:", pexErr.message);
        }
    }

    // 3. High-Speed Studio AI Fallback (Pollinations with Commercial Studio Directives)
    if (!imageBuffer) {
        try {
            const cleanStudioPrompt = encodeURIComponent(
                `High-end commercial advertising poster for ${service} by ${businessName} in ${industry}. Sleek modern commercial studio lighting, polished reflective obsidian pedestal, dynamic perspective, vibrant colors, 8k render, masterpiece.`
            );
            const pollUrl = `https://image.pollinations.ai/prompt/${cleanStudioPrompt}?width=1024&height=1024&nologo=true&seed=${Math.floor(Math.random() * 1000000)}`;
            ...
            console.warn("[Image Generation] Pollinations fallback failed:", pollErr.message);
        }
    }
