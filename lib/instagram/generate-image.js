...
export async function generateImage(state) {
    ...
    // Resolve Brand Elements from state (synced from Supabase / Meta)
    ...
    let businessPhone = "";
    if (state.assets.phone) businessPhone = state.assets.phone;
    else if (method === "website" && state.assets.websiteUrl) businessPhone = state.assets.websiteUrl;
    else if ((method === "phone" || method === "whatsapp") && state.assets.phone) businessPhone = state.assets.phone;

    // Resolve logo availability
    const logoAvailable = (state.assets.logoDecision === "use_logo" && state.assets.logoUrl) ? "YES" : "NO";

    // Resolve offer text
    const offerText = (offer && offer.toLowerCase() !== "none") ? offer : "None";

    // Resolve phone text
    const phoneText = businessPhone || "None";

    // Build LOGO section
    let logoSection = "";
    if (logoAvailable === "YES") {
        logoSection = `If Logo Available = YES
→ place the provided logo here using this reference: ${state.assets.logoUrl}`;
    } else {
        logoSection = `If Logo Available = NO
→ place the text "${businessName}" in elegant clean typography inside this area`;
    }

    // Build OFFER BADGE section
    let offerBadgeSection = "";
    if (offerText !== "None") {
        offerBadgeSection = `
...
`;
    ...
    // Build CONTACT AREA section
    let contactAreaSection = "";
    if (phoneText !== "None") {
        contactAreaSection = `If Business Phone exists
→ display text:

Contact: ${phoneText}`;
    } else {
        contactAreaSection = `If Business Phone does NOT exist
→ display:
...
DM us`;
    ...
    // ═══════════════════════════════════════════════════════════════════
    // THE EXACT DALL-E PROMPT — variables filled from user's synced data
    // ═══════════════════════════════════════════════════════════════════
...
A professional Instagram marketing creative representing "${service}" for "${businessName}" following the layout constraints above.`;
    ...
    console.log(`🖼️ [Image Gen] Prompt length: ${prompt.length} chars`);
    console.log(`🖼️ [Image Gen] Business: ${businessName} | Service: ${service} | Offer: ${offerText} | Phone: ${phoneText} | Logo: ${logoAvailable}`);
