...
export async function processImage({
    ...
    try {
        ...
        // Priority: 1) AI layout_hint  2) Time-slot rotation (guarantees variety for consecutive posts)
        // Time-slot: divide the session ID (timestamp) into 5-minute windows, mod 5 → always rotates A→B→C→D→E
        let templateId;
        if (layoutHint && LAYOUTS.includes(layoutHint)) {
            templateId = layoutHint;
        } else {
            // Use session timestamp to derive slot so consecutive posts ALWAYS differ
            const timeSlot = Math.floor(Date.now() / (5 * 60 * 1000)); // changes every 5 min
            const sessionSeed = sessionId ? sessionId.replace(/\D/g, "").slice(-6) : "0";
            const combined = (timeSlot + parseInt(sessionSeed || "0", 10)) % LAYOUTS.length;
            templateId = LAYOUTS[combined];
        }
        // Accent: rotate through 7 colors, seeded by current minute so each post differs
        const accentIdx = Math.floor(Date.now() / (2 * 60 * 1000)) % ACCENT_COLORS.length;
        ...
        // ═══════════════════════════════════════════════════════════════
            ...
            const svcLines = wrapText(service.toUpperCase(), 56, 900);
            const tagLines = wrapText(tagline, 24, 900);

            const panelY = 580;
            let ty = panelY + 64;
            const parts = [];
            svcLines.forEach(l => { parts.push(getPath(l, 512, ty, 56, "center", "#FFFFFF")); ty += 68; });
            ty += 6;
            tagLines.forEach(l => { parts.push(getPath(l, 512, ty, 24, "center", "#CCCCCC", true)); ty += 30; });
            if (offer && offer !== "None") {
                ty += 16;
            ...
            overlaySVG = `<svg width="1024" height="1024" xmlns="http://www.w3.org/2000/svg">
                ...
                <rect x="0" y="${panelY}" width="1024" height="${1024 - panelY}" fill="black" fill-opacity="0.84"/>
                <rect x="0" y="${panelY}" width="1024" height="4" fill="${accentColor}"/>
                ${parts.join("\n")}
                ${getPath(contactDisplay, 512, 1006, 21, "center", "#999999")}
            </svg>`;
        }
