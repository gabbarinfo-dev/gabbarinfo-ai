...
export async function generateCaption(state) {
  ...
  try {
    ...
    const ctaLine = website
      ? `👉 Visit ${website}`
      : phone
        ? state.assets.contactMethod === "whatsapp"
          ? `👉 WhatsApp us now`
          : `👉 Call ${phone}`
        : "👉 DM us for details";
...
`;
    ...
    if (!jsonMatch) {
      throw new Error("Caption resolution failed (invalid response).");
    }

    const data = JSON.parse(jsonMatch[0]);
    if (!data.caption) {
      throw new Error("Caption resolution failed (no text).");
    }
    ...
    const fallbackCaption = `Experience top-tier ${state.context.service} with ${state.businessName}. ${
      state.context.offer
        ? `Don't miss out: ${state.context.offer}!`
