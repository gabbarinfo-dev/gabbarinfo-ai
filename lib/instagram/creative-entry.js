...
export async function creativeEntry({ supabase, session, instruction, metaRow, effectiveBusinessId, targetPlatform = "instagram" }) {
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.CONTENT_GENERATION) {
            // 1. Generate Caption & Tagline
            const captionData = await generateCaption(state);

            // 2. Generate Masterpiece Ad Graphic using DALL-E 3 HD with Dynamic Art Direction
            const imageData = await generateImage(state, captionData.visualMood, captionData.tagline);
        ...
        if (state.stage === CREATIVE_STAGES.PREVIEW) {
            const isConfirmation = lowerInstruction.match(/\b(yes|publish|go ahead|confirm|ok|proceed|option 1|option 2|both|facebook|instagram)\b/i);
            ...
            if (isConfirmation) {
                ...
                if (
                    ...
                    (state.targetPlatform === "facebook" && (lowerInstruction.includes("option 1") || lowerInstruction.includes("facebook")))
                ) {
                    destination = "FACEBOOK_ONLY";
                } else if (
                    lowerInstruction.includes("only instagram") ||
                    lowerInstruction.includes("instagram only") ||
                    (state.targetPlatform === "instagram" && (lowerInstruction.includes("option 1") || lowerInstruction.includes("instagram")))
                ) {
            ...
            if (wantsBothChange) {
                ...
                await saveCreativeState(supabase, email, currentSessionId, state);
                return { response: { ok: true, text: "Got it. Regenerating both image and caption based on your feedback..." } };
                ...
                // General feedback — regenerate both as fallback
                state.stage = CREATIVE_STAGES.CONTENT_GENERATION;
                state.context.rawIntent += " " + instruction;
                await saveCreativeState(supabase, email, currentSessionId, state);
