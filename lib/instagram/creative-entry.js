...
export async function creativeEntry({ supabase, session, instruction, metaRow, effectiveBusinessId, targetPlatform = "instagram" }) {
    ...
    const isNewIntent = lowerInstruction.includes("publish an instagram") ||
        lowerInstruction.includes("publish a facebook") ||
        lowerInstruction.includes("publish a post") ||
        lowerInstruction.includes("create an instagram") ||
        lowerInstruction.includes("create a facebook") ||
        lowerInstruction.includes("post to instagram") ||
        lowerInstruction.includes("post to facebook");
    ...
    if (isReset || isNewIntent || state.stage === CREATIVE_STAGES.COMPLETED) {
        ...
        const nextSessionId = `${targetPlatform || "ig"}_creative_${Date.now()}`;
        state = {
    ...
    if (!state.targetPlatform) {
        state.targetPlatform = targetPlatform || "instagram";
    }
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT_WAITING) {
            ...
            if (!serviceInput || serviceInput.length < 3) {
                const pLabel = state.targetPlatform === "facebook" ? "Facebook" : "Instagram";
                return { response: { ok: true, text: `What service or product do you want this ${pLabel} post to focus on?` } };
            }
        ...
        if (state.stage === CREATIVE_STAGES.CONTENT_GENERATION) {
            // Run caption generation and image generation concurrently to eliminate latency & prevent serverless timeouts
            ...
            state.content = {
                ...
                storageFileName: imageData.storageFileName
            };
        ...
        if (state.stage === CREATIVE_STAGES.PREVIEW) {
            const isConfirmation = lowerInstruction.match(/\b(yes|yeah|yep|sure|publish|post|post it|post now|publish now|go ahead|confirm|ok|okay|proceed|option 1|option 2|option1|option2|both|facebook|instagram|looks good|looks great|perfect|approved|approve|done|share|send|upload|ready)\b|^1$|^2$/i);
            ...
            if (isConfirmation) {
                ...
                let destination = state.targetPlatform === "facebook" ? "FACEBOOK_ONLY" : "INSTAGRAM_ONLY";
                if (
                    lowerInstruction.includes("both") ||
                    lowerInstruction.includes("option 2") ||
                    lowerInstruction.includes("option2") ||
                    lowerInstruction.trim() === "2" ||
                    lowerInstruction.includes("cross") ||
                    (lowerInstruction.includes("facebook") && lowerInstruction.includes("instagram"))
                ) {
                    destination = "BOTH";
                } else if (
                    lowerInstruction.includes("only facebook") ||
                    lowerInstruction.includes("facebook only") ||
                    (state.targetPlatform === "facebook" && (lowerInstruction.includes("option 1") || lowerInstruction.includes("option1") || lowerInstruction.trim() === "1" || lowerInstruction.includes("facebook")))
                ) {
                    destination = "FACEBOOK_ONLY";
                } else if (
                    lowerInstruction.includes("only instagram") ||
                    lowerInstruction.includes("instagram only") ||
                    (state.targetPlatform === "instagram" && (lowerInstruction.includes("option 1") || lowerInstruction.includes("option1") || lowerInstruction.trim() === "1" || lowerInstruction.includes("instagram")))
                ) {
                    destination = "INSTAGRAM_ONLY";
                }
                ...
                return {
                    assets: {
                        ...
                        storageFileName: state.content.storageFileName, // Expose this for cleanup
                        destination: destination,
                        targetPlatform: state.targetPlatform || "instagram"
                    }
                };
            }

            // --- FLEXIBLE REVISION COMMANDS ---
            const wantsCaptionChange = lowerInstruction.match(/\b(change caption|edit caption|update caption|new caption|rewrite caption|redo caption|fix caption)\b/i);
            const wantsCaptionAdd = lowerInstruction.match(/\b(add in caption|add to caption|append to caption|include in caption)\b/i);
            const wantsImageChange = lowerInstruction.match(/\b(change image|new image|regenerate image|redo image|different image|update image|recreate image)\b/i);
            const wantsHashtagChange = lowerInstruction.match(/\b(change hashtags|new hashtags|update hashtags|fix hashtags|add hashtags|relevant hashtags)\b/i);
            const wantsBothChange = lowerInstruction.match(/\b(change both|redo both|regenerate both|start over|change everything|change all)\b/i);
            const isCorrection = !isConfirmation && instruction.length > 5;

            if (wantsBothChange || isCorrection) {
                // Regenerate both caption and image based on user's feedback
                state.stage = CREATIVE_STAGES.CONTENT_GENERATION;
                state.context.rawIntent += " User revision: " + instruction;
                try {
                    const [captionData, imageData] = await Promise.all([
                        generateCaption(state),
                        generateImage(state)
                    ...
                    await saveCreativeState(supabase, email, currentSessionId, state);
                    const previewText = composePreview(state);
                    return { response: { ok: true, text: `✅ Content updated with your feedback!\n\n${previewText}` } };
                } catch (genErr) {
                    console.error("Revision generation error:", genErr);
                    await saveCreativeState(supabase, email, currentSessionId, state);
                    return { response: { ok: false, text: `Failed to regenerate creative: ${genErr.message}` } };
                }

            } else if (wantsCaptionAdd) {
                // Append user's text to existing caption
                const addText = instruction.replace(/add (in|to) caption[:\s]*/i, "").trim();
                if (addText) {
                    state.content.caption = (state.content.caption || "") + "\n" + addText;
                }
                await saveCreativeState(supabase, email, currentSessionId, state);
                const previewText = composePreview(state);
                return { response: { ok: true, text: "✅ Caption updated!\n\n" + previewText } };

            } else if (wantsCaptionChange || wantsHashtagChange) {
                // Regenerate caption only, keep image
                state.context.rawIntent += " " + instruction;
                try {
                    const captionData = await generateCaption(state);
                    state.content.caption = captionData.caption;
                    state.content.hashtags = captionData.hashtags;
                } catch (e) {
                    console.error("Caption regeneration error:", e);
                }
                await saveCreativeState(supabase, email, currentSessionId, state);
                const previewText = composePreview(state);
                return { response: { ok: true, text: "✅ Content updated!\n\n" + previewText } };

            } else if (wantsImageChange) {
                // Regenerate image only, keep caption
                state.context.rawIntent += " " + instruction;
                try {
                    // Refresh the mood first
                    const captionData = await generateCaption(state);
                    const imageData = await generateImage(state, captionData.visualMood);
                    state.content.imageUrl = imageData.imageUrl;
                    state.content.imagePrompt = imageData.imagePrompt;
                } catch (e) {
                    console.error("Image regeneration error:", e);
                }
                await saveCreativeState(supabase, email, currentSessionId, state);
                const previewText = composePreview(state);
                return { response: { ok: true, text: "✅ Image updated!\n\n" + previewText } };
            }
