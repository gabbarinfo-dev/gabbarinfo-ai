...
function isMeaninglessConfirmation(instruction) {
    const text = instruction.toLowerCase().trim();
    // Common low-signal affirmations
    const meaningless = [
        "ok", "okay", "yes", "yeah", "alright", "all right", "sure", "fine", "yep", "yup", "ok cool", "cool"
    ];
    return meaningless.includes(text);
}
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.BUSINESS_RESOLUTION_WAITING) {
            ...
            if (instruction.match(/\b(yes|ok|sure|this one|confirm)\b/i)) {
                ...
                state.businessName = brandName;
                state.stage = CREATIVE_STAGES.SERVICE_CONTEXT;
                await saveCreativeState(supabase, email, creativeSessionId, state);
                return { response: { ok: true, text: `Great. Now, what service are we highlighting for ${brandName}?` } };
            }
            return { response: { ok: true, text: `I'm sorry, I didn't catch that. Should I use your connected Instagram account for ${brandName}?` } };
        }
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT_WAITING) {
            if (isMeaninglessConfirmation(instruction)) {
                const ctxResult = resolveContext(state);
                return { response: { ok: true, text: ctxResult.question } };
            }
            ...
            state.stage = CREATIVE_STAGES.OFFER_CONTEXT;
            await saveCreativeState(supabase, email, creativeSessionId, state);
            return { response: { ok: true, text: `Got it. And any specific offer or discount you'd like to mention for ${state.context.service}?` } };
        }
        ...
        if (state.stage === CREATIVE_STAGES.OFFER_CONTEXT_WAITING) {
            ...
            state.stage = CREATIVE_STAGES.CONTACT_PREFERENCE;
            await saveCreativeState(supabase, email, creativeSessionId, state);
            return { response: { ok: true, text: `Understood. How should customers reach out to ${brandName}? Website, Call, or WhatsApp?` } };
        }
        ...
        if (state.stage === CREATIVE_STAGES.CONTACT_PREFERENCE_WAITING) {
            ...
            if (method) {
                ...
                state.stage = CREATIVE_STAGES.ASSET_CONFIRMATION;
                await saveCreativeState(supabase, email, creativeSessionId, state);
                return { response: { ok: true, text: "All set. Should I generate the post now?" } };
            }
            if (isMeaninglessConfirmation(instruction)) {
                const assetResult = await resolveAssets(supabase, state, metaRow);
                return { response: { ok: true, text: assetResult.question } };
            }
            return { response: { ok: true, text: "Please choose a contact method: Website, Call, WhatsApp, or None." } };
        }
        ...
        if (state.stage === CREATIVE_STAGES.ASSET_CONFIRMATION_WAITING) {
            ...
            if (state.assets.assetsConfirmed) {
                await saveCreativeState(supabase, email, creativeSessionId, state);
                return { response: { ok: true, text: "Got it. Moving on to content creation." } };
            }
            if (isMeaninglessConfirmation(instruction)) {
                const assetResult = await resolveAssets(supabase, state, metaRow);
                return { response: { ok: true, text: assetResult.question } };
            }
            return { response: { ok: true, text: `I still need that information to finalize the post for ${brandName}. Could you provide it?` } };
        }
        ...
        if (state.stage === CREATIVE_STAGES.LOGO_DECISION_PROCESS) {
            ...
            await saveCreativeState(supabase, email, creativeSessionId, state);
            return { response: { ok: true, text: "Generating your post..." } };
        }
        ...
        if (state.stage === CREATIVE_STAGES.PREVIEW) {
            const isConfirmation = instruction.match(/\b(yes|publish|confirm|go ahead)\b/i);


            if (isConfirmation) {
                // 1. Construct exact payload
                const finalCaption = `${state.content.caption}\n\n${state.content.hashtags.join(" ")}`.trim();
                const payload = {
                ...
                // 2. Add temporary log
                console.log("[CREATIVE → PATH A]", payload);


                // 3. Hard guard before return
                if (!payload.imageUrl || !payload.caption) {
                    throw new Error("Creative payload incomplete: imageUrl or caption missing");
                }


                // 4. Final Lifecycle Step (Order: After construction, before return)
                state.stage = CREATIVE_STAGES.COMPLETED;
                await saveCreativeState(supabase, email, creativeSessionId, state);


                // 5. Final Intent Return
                return {
