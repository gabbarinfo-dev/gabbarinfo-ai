...
export async function creativeEntry({ supabase, session, instruction, metaRow, effectiveBusinessId }) {
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT) {
            ...
            // FIX: Check the result directly. State merge might be shallow or delayed.
            // We check if 'serviceLocked' is present in the updates we just got, 
            // OR if it was already locked in the state (recovery).
            const isServiceLocked = ctxResult.updates?.context?.serviceLocked || state.context?.serviceLocked;
            ...
            // Update state to allow fall-through to next stage in same turn
            state = { ...state, ...updates };
        }
