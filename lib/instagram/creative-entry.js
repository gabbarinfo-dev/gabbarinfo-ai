...
export async function creativeEntry({ supabase, session, instruction, metaRow, effectiveBusinessId }) {
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.PREVIEW) {
            ...
            const wantsCaptionChange = lowerInstruction.match(/\b(change caption|edit caption|update caption|new caption|rewrite caption|redo caption)\b/i);
            const wantsCaptionAdd = lowerInstruction.match(/\b(add in caption|add to caption|append to caption|include in caption)\b/i);
            const wantsImageChange = lowerInstruction.match(/\b(change image|new image|regenerate image|redo image|different image|update image)\b/i);
            const wantsBothChange = lowerInstruction.match(/\b(change both|redo both|regenerate both|start over|change everything)\b/i);
            const isCorrection = instruction.length > 5;
            ...
            if (wantsBothChange) {
                ...
                state.stage = CREATIVE_STAGES.CONTENT_GENERATION;
                await saveCreativeState(supabase, email, currentSessionId, state);
                return { response: { ok: true, text: "Got it. Regenerating both image and caption..." } };
            ...
            } else if (wantsCaptionChange && !wantsImageChange) {
            ...
            } else if (wantsImageChange && !wantsCaptionChange) {
                ...
                // General feedback — regenerate both
                state.stage = CREATIVE_STAGES.CONTENT_GENERATION;
                state.context.rawIntent += " " + instruction;
                await saveCreativeState(supabase, email, currentSessionId, state);
