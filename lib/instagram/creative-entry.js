import { CREATIVE_STAGES, CREATIVE_INTENT, DEFAULT_CREATIVE_STATE }
...
const email = session.user.email.toLowerCase();
 ...
 const isGenericTrigger = instruction.toLowerCase().trim() ===
"publish an instagram post";
 if (isGenericTrigger && !hasImage && !hasCaption) {
 console.log("🔄 [Creative] Generic trigger detected. Resetting
to fresh state.");
 state = {
...
creativeSessionId, state);
 ...
 if (state.stage ===
CREATIVE_STAGES.BUSINESS_RESOLUTION_WAITING) {
 // FIX: Selection from metaRow only (No Meta Graph API
network calls)
     if (instruction.match(/\b(yes|ok|sure|this
one|confirm)\b/i)) {
 state.businessId = metaRow.instagram_actor_id ||
metaRow.ig_business_id;
 state.businessName = metaRow?.business_name || "Your
Business"; // Professional fallback branding
 state.stage = CREATIVE_STAGES.SERVICE_CONTEXT;
 await saveCreativeState(supabase, email,
creativeSessionId, state);
 return { response: { ok: true, text: "Great. Now, what
service are we highlighting?" } };
 }
 return { response: { ok: true, text: "I didn't quite get
that. To clarify, should I use your connected Instagram handle to
publish?" } };
 }
 ...
 await saveCreativeState(supabase, email,
creativeSessionId, state);
 return { response: { ok: true, text: "Can you specify
the exact service for this post? (e.g., Necklace Set, Bridal
Jewellery)" } };
 }
 }
 ...
 await saveCreativeState(supabase, email, creativeSessionId,
state);
 return { response: { ok: true, text: "Got it. And any
specific offer or discount for this post?" } };
 ...
 await saveCreativeState(supabase, email,
creativeSessionId, state);
 return { response: { ok: true, text: "Please share the
exact offer text you want shown on the post." } };
 }
 }
 ...
 await saveCreativeState(supabase, email, creativeSessionId,
state);
 return { response: { ok: true, text: "Understood. How
should people reach out? Website, Call, or WhatsApp?" } };
 }
...
CREATIVE_STAGES.CONTACT_PREFERENCE_WAITING;
 await saveCreativeState(supabase, email,
creativeSessionId, state);
 return { response: { ok: true, text: "Confirming —
should customers contact you via Website, Call, WhatsApp, or None?" }
};
 }
 }
...
method = "phone";
 ...
 await saveCreativeState(supabase, email,
creativeSessionId, state);
 return { response: { ok: true, text: "Noted. Should I
generate the post now?" } };
 }
 return { response: { ok: true, text: "Confirming — should
customers contact you via Website, Call, WhatsApp, or None?" } };
 }
...
urlMatch[0].startsWith("http") ? urlMatch[0] : "https://" +
urlMatch[0];
...
instruction.match(/(\+?\d[\d\s-]{8,})/);
 ...
 await saveCreativeState(supabase, email,
creativeSessionId, state);
 return { response: { ok: true, text: "Got it. Moving on
to your logo and branding setup." } };
 }
 return { response: { ok: true, text: "I still need those
contact details to finalize the post. Could you provide them again?" }
};
 }
...
assetResult.question } };
 ...
 // FIX: Split Authority - Assignments moved to PROCESS
stage
 const finalLogo = state.assets.logoUrl || metaRow?.logo ||
metaRow?.logo_url;
...
`${state.content.caption}\n\n${state.content.hashtags.join(" ")}`
 ...
 await saveCreativeState(supabase, email, creativeSessionId,
state);
 return { response: { ok: true, text: "Updating based on
your feedback..." } };
 }
...
creativeSessionId);
 ...
 return { response: { ok: false, text: `Creative Mode Error:
${e.message}` } };
 }
