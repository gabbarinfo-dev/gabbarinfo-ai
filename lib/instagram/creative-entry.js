...
export async function creativeEntry({ supabase, session, instruction, metaRow, effectiveBusinessId }) {
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.BUSINESS_RESOLUTION_WAITING) {
            const isConfirm = lowerInstruction.includes("yes") || lowerInstruction.includes("sure") || lowerInstruction.includes("ok");
            if (isConfirm) {
                const nextState = {
                    ...state,
                    businessId: "confirmed", // Signal resolution
                    stage: CREATIVE_STAGES.SERVICE_CONTEXT
                };
                await saveCreativeState(supabase, email, creativeSessionId, nextState);
                state = nextState;
                // Flow through to Service Context
            } else {
