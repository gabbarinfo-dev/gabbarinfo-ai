...
export async function creativeEntry({ supabase, session, instruction, metaRow, effectiveBusinessId }) {
    ...
    if (instruction.match(/\b(cancel|stop|start over|reset)\b/i)) {
        ...
        return {
            response: {
                ok: true, text: "Creative mode canceled. How can I help?"
            }
        };
    }
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.BUSINESS_RESOLUTION) {
            ...
            if (bizResult.complete) {
                ...
                return {
                    response: {
                        ok: true, text: bizResult.question
                    }
                };
            }
        }
        ...
        if (state.stage === CREATIVE_STAGES.BUSINESS_RESOLUTION_WAITING) {
            ...
            if (instruction.match(/\b(yes|ok|sure|this one|confirm)\b/i)) {
                ...
                return {
                    response: {
                        ok: true, text: "Great. Now, what service are we highlighting?"
                    }
                };
            }
            return {
                response: {
                    ok: true, text: "I didn't quite get that. To clarify, should I use your connected Instagram handle to publish?"
                }
            };
        }
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT) {
            ...
            if (ctxResult.complete) {
                ...
                return {
                    response: {
                        ok: true, text: "Can you specify the exact service for this post? (e.g., Necklace Set, Bridal Jewellery)"
                    }
                };
            }
        }
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT_WAITING) {
            ...
            return {
                response: {
                    ok: true, text: "Got it. And any specific offer or discount for this post?"
                }
            };
        }
        ...
        if (state.stage === CREATIVE_STAGES.OFFER_CONTEXT) {
            ...
            if (ctxResult.complete) {
                ...
                return {
                    response: {
                        ok: true, text: "Please share the exact offer text you want shown on the post."
                    }
                };
            }
        }
        ...
        if (state.stage === CREATIVE_STAGES.OFFER_CONTEXT_WAITING) {
            ...
            return {
                response: {
                    ok: true, text: "Understood. How should people reach out? Website, Call, or WhatsApp?"
                }
            };
        }
        ...
        if (state.stage === CREATIVE_STAGES.CONTACT_PREFERENCE) {
            ...
            if (assetResult.complete) {
                ...
                return {
                    response: {
                        ok: true, text: "Confirming — should customers contact you via Website, Call, WhatsApp, or None?"
                    }
                };
            }
        }
        ...
        if (state.stage === CREATIVE_STAGES.CONTACT_PREFERENCE_WAITING) {
            ...
            if (method) {
                ...
                return {
                    response: {
                        ok: true, text: "Noted. Should I generate the post now?"
                    }
                };
            }
            return {
        ...
        if (state.stage === CREATIVE_STAGES.ASSET_CONFIRMATION_WAITING) {
            ...
            if (state.assets.assetsConfirmed) {
                ...
                return {
                    response: {
                        ok: true, text: "Got it. Moving on to your logo and branding setup."
                    }
                };
            }
            return {
                response: {
                    ok: true, text: "I still need those contact details to finalize the post. Could you provide them again?"
                }
            };
        }
        ...
        if (state.stage === CREATIVE_STAGES.LOGO_DECISION) {
            ...
            if (assetResult.complete) {
                ...
                return {
                    response: {
                        ok: true, text: assetResult.question
                    }
                };
            }
        }
        ...
        if (state.stage === CREATIVE_STAGES.PREVIEW) {
            ...
            return {
                response: {
                    ok: true, text: "Updating based on your feedback..."
                }
            };
        }
        ...
        return {
            response: {
                ok: false, text: `Creative Mode Error: ${e.message}`
            }
        };
    }
