...
import { CREATIVE_STAGES, DEFAULT_CREATIVE_STATE } from 
"./creative-constants"; 
import { loadCreativeState, saveCreativeState, clearCreativeState } 
...
export async function creativeEntry({ supabase, session, instruction, 
metaRow }) { 
    ...
    if (!creativeSessionId) { 
        ...
        await saveCreativeState(supabase, email, creativeSessionId, 
state); 
    ...
    if (instruction.match(/\b(cancel|stop|start over|reset)\b/i)) { 
        ...
        return { response: { ok: true, text: "Creative mode canceled. 
How can I help?", mode: "instagram_post" } }; 
    } 
 
    try { 
        // --- STAGE 1: BUSINESS RESOLUTION (Decision) --- 
        if (state.stage === CREATIVE_STAGES.BUSINESS_RESOLUTION) { 
            const bizResult = await resolveBusiness(session, metaRow, 
state); 
            if (bizResult.complete) { 
                state.stage = CREATIVE_STAGES.SERVICE_CONTEXT; 
                await saveCreativeState(supabase, email, 
creativeSessionId, state); 
                return { response: { ok: true, text: "Business already 
selected. Proceeding to service details.", mode: "instagram_post" } }; 
            } else { 
                state.stage = 
...
bizResult.question, mode: "instagram_post" } }; 
            } 
        } 
...
CREATIVE_STAGES.BUSINESS_RESOLUTION_WAITING) { 
            ...
            const url = 
`https://graph.facebook.com/v21.0/me/accounts?fields=name,category,id,i
nstagram_business_account{id,username,profile_picture_url}&access_token
=${accessToken}`; 
            const res = await fetch(url); 
            const data = await res.json(); 
            const validAccounts = (data.data || []).filter(p => 
...
(lowerInst.includes("yes") || lowerInst.includes("sure") || 
lowerInst.includes("ok"))) 
            ); 
 
            if (matched) { 
                // SINGLE AUTHORITY assignment 
                state.businessId = 
matched.instagram_business_account.id; 
                state.businessName = 
matched.instagram_business_account.username; 
                state.businessCategory = matched.category || 
metaRow?.category || null; 
 
                state.stage = CREATIVE_STAGES.SERVICE_CONTEXT; 
                await saveCreativeState(supabase, email, 
creativeSessionId, state); 
                return { response: { ok: true, text: `Account 
@${state.businessName} selected. Next: Service details.`, mode: 
"instagram_post" } }; 
            } else { 
                return { response: { ok: true, text: "I couldn't match 
that account. Please provide the username or name mentioned above.", 
mode: "instagram_post" } }; 
            } 
        } 
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT) { 
            ...
            if (ctxResult.complete) { 
                ...
                await saveCreativeState(supabase, email, 
creativeSessionId, state); 
                return { response: { ok: true, text: "Service 
finalized. Next: Offer details.", mode: "instagram_post" } }; 
            } else { 
...
creativeSessionId, state); 
                return { response: { ok: true, text: 
ctxResult.question, mode: "instagram_post" } }; 
            } 
        } 
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT_WAITING) { 
            ...
            await saveCreativeState(supabase, email, creativeSessionId, 
state); 
            return { response: { ok: true, text: "Service noted. Let's 
discuss the offer.", mode: "instagram_post" } }; 
        } 
        ...
        if (state.stage === CREATIVE_STAGES.OFFER_CONTEXT) { 
            ...
            if (ctxResult.complete) { 
                ...
                await saveCreativeState(supabase, email, 
creativeSessionId, state); 
                return { response: { ok: true, text: "Offer finalized. 
Next: Contact preferences.", mode: "instagram_post" } }; 
            } else { 
...
ctxResult.question, mode: "instagram_post" } }; 
        ...
        if (state.stage === CREATIVE_STAGES.OFFER_CONTEXT_WAITING) { 
            ...
            const hasOffer = 
!offerText.match(/\b(none|no|skip|nothing|na)\b/i); 
            ...
            await saveCreativeState(supabase, email, creativeSessionId, 
state); 
            return { response: { ok: true, text: "Offer captures. 
Proceeding to contact methods.", mode: "instagram_post" } }; 
        } 
 
        // --- STAGE 4: CONTACT_PREFERENCE (Decision) --- 
        if (state.stage === CREATIVE_STAGES.CONTACT_PREFERENCE) { 
            const assetResult = await resolveAssets(supabase, state, 
metaRow); 
            if (assetResult.complete) { 
                state.stage = CREATIVE_STAGES.ASSET_CONFIRMATION; 
                await saveCreativeState(supabase, email, 
creativeSessionId, state); 
                return { response: { ok: true, text: "Checking stored 
assets.", mode: "instagram_post" } }; 
            } else { 
                state.stage = 
...
creativeSessionId, state); 
                return { response: { ok: true, text: 
assetResult.question, mode: "instagram_post" } }; 
            } 
        } 
...
method = "phone"; 
            ...
            await saveCreativeState(supabase, email, creativeSessionId, 
state); 
            return { response: { ok: true, text: `Contact method set to 
${method}. Moving to verification.`, mode: "instagram_post" } }; 
        } 
...
Finalizing design.", mode: "instagram_post" } }; 
            } else { 
                state.stage = 
...
{ 
            ...
            let website = state.assets.websiteUrl || (method === 
"website" ? 
instruction.match(/(https?:\/\/[^\s]+|www\.[^\s]+|[a-zA-Z0-9-]+\.[a-z]{
2,})/i)?.[0] : null); 
            let phone = state.assets.phone || ((method === "phone" || 
method === "whatsapp") ? instruction.match(/(\+?\d[\d\s-]{8,})/)?.[0] : 
null); 
 
            state.assets.websiteUrl = website || 
state.assets.websiteUrl; 
            state.assets.phone = phone || state.assets.phone; 
            state.assets.assetsConfirmed = (method === "website" ? 
!!website : (method === "phone" || method === "whatsapp" ? !!phone : 
true)); 
            ...
            await saveCreativeState(supabase, email, creativeSessionId, 
state); 
            return { response: { ok: true, text: "Asset data updated.", 
mode: "instagram_post" } }; 
        } 
        ...
        if (state.stage === CREATIVE_STAGES.LOGO_DECISION) { 
            ...
            let finalLogo = state.assets.logoUrl || metaRow?.logo || 
metaRow?.logo_url; 
            state.assets.logoUrl = finalLogo || null; 
            state.assets.logoDecision = finalLogo ? "use_logo" : 
"use_text"; 
            ...
            await saveCreativeState(supabase, email, creativeSessionId, 
state); 
            return { response: { ok: true, text: "Generating creative 
content...", mode: "instagram_post" } }; 
        } 
...
state); 
            ...
            // HARD RETURN: No fallthrough, return preview text 
directly 
            const previewText = composePreview(state); 
            return { response: { ok: true, text: previewText, mode: 
"instagram_post" } }; 
        } 
 
        // --- STAGE 8: PREVIEW & PUBLISH (Wait for confirmation) --- 
        if (state.stage === CREATIVE_STAGES.PREVIEW) { 
            const isConfirmation = instruction.match(/\b(yes|publish|go 
ahead|confirm)\b/i); 
            if (isConfirmation) { 
                // 
�
...
"instagram_post" } }; 
        ...
        if (state.stage === CREATIVE_STAGES.COMPLETED) { 
            await clearCreativeState(supabase, email, 
creativeSessionId); 
            return { response: { ok: true, text: "Post flow 
completed.", mode: "instagram_post" } }; 
        } 
 
        return { response: { ok: false, text: "FSM internal limit 
reached.", mode: "instagram_post" } }; 
 
    } catch (e) { 
        console.error("Creative Entry Error:", e); 
        return { response: { ok: false, text: `FSM Error: 
${e.message}`, mode: "instagram_post" } }; 
    } 
} 
