...
export async function creativeEntry({ supabase, session, instruction, metaRow, effectiveBusinessId }) {
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.CONTENT_GENERATION) {
            // Sequential generation so Image can use Mood from Caption
            const captionData = await generateCaption(state);
            const imageData = await generateImage(state, captionData.visualMood);

            // NEW: Perfect Text Overlay Processing
            let finalImageUrl = imageData.imageUrl;
            let finalFileName = null;

            try {
                const { processImage } = await import("./process-image");
                
                // Resolve contact text and method for the image overlay
                let contactText = "DM us";
                const method = state.assets.contactMethod;
                
                if (method === "website") {
                    contactText = state.assets.websiteUrl || "link in bio";
                } else if (method === "phone" || method === "whatsapp") {
                    contactText = state.assets.phone || "call us";
                }

                // --- GUARANTEED LAYOUT ROTATION (persistent counter per user) ---
                // Always advances to next template: A→B→C→D→E→A, regardless of timing.
                const ROTATION = ["A", "B", "C", "D", "E"];
                let forcedLayout = captionData.layoutHint || "A";
                try {
                    const { data: rotRow } = await supabase
                        .from("agent_memory")
                        .select("content")
                        .eq("email", email)
                        .eq("memory_type", "ig_layout_idx")
                        .maybeSingle();

                    const lastIdx = rotRow?.content != null ? parseInt(rotRow.content, 10) : -1;
                    const nextIdx = (lastIdx + 1) % ROTATION.length;
                    forcedLayout = ROTATION[nextIdx];

                    await supabase.from("agent_memory").upsert({
                        email: email,
                        memory_type: "ig_layout_idx",
                        content: String(nextIdx),
                        updated_at: new Date().toISOString()
                    }, { onConflict: "email,memory_type" });

                    console.log(`[Layout] last=${lastIdx} → next=${nextIdx} → template=${forcedLayout}`);
                } catch (rotErr) {
                    console.warn("[Layout] Rotation read failed:", rotErr.message);
                }
                ...
                const processed = await processImage({
                    ...
                    sessionId: state.creativeSessionId,
                    layoutHint: forcedLayout
                });

                finalImageUrl = processed.finalImageUrl;
                finalFileName = processed.fileName;
            } catch (err) {
                console.error("[CreativeEntry] Overlay failed, falling back to original:", err);
            }
            ...
            state.content = {
                ...
                storageFileName: finalFileName // Store this for later deletion
            };
