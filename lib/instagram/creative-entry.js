...
export async function creativeEntry({ supabase, session, instruction, metaRow, effectiveBusinessId }) {
    ...
    try {
        ...
        if (state.stage === CREATIVE_STAGES.BUSINESS_RESOLUTION) {
            const bizResult = await resolveBusiness(session, metaRow, inputConsumed ? "" : instruction, state);
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT) {
            const ctxResult = resolveContext(inputConsumed ? "" : instruction, state);
        ...
        if (state.stage === CREATIVE_STAGES.SERVICE_CONTEXT_WAITING && !inputConsumed) {
            ...
            return { response: { ok: true, text: "Got it. Let's continue.", mode: "instagram_post" } };
        }
        ...
        if (state.stage === CREATIVE_STAGES.OFFER_CONTEXT_WAITING && !inputConsumed) {
            ...
            return { response: { ok: true, text: "Understood.", mode: "instagram_post" } };
        }

        // --- STAGE 4: CONTACT_PREFERENCE ---
        if (state.stage === CREATIVE_STAGES.CONTACT_PREFERENCE) {
            const assetResult = await resolveAssets(supabase, state, inputConsumed ? "" : instruction, metaRow);
        ...
        if (state.stage === CREATIVE_STAGES.CONTACT_PREFERENCE_WAITING && !inputConsumed) {
            ...
            if (method) {
                ...
                return { response: { ok: true, text: "Noted.", mode: "instagram_post" } };
            } else {
                return { response: { ok: true, text: "Please choose one: 'Website', 'Call', 'WhatsApp', or 'None'.", mode: "instagram_post" } };
            }
        }
        ...
        if (state.stage === CREATIVE_STAGES.ASSET_CONFIRMATION_WAITING && !inputConsumed) {
            ...
            if (assetResult.complete || assetResult.updates?.assets?.assetsConfirmed) {
                ...
                return { response: { ok: true, text: "Assets confirmed.", mode: "instagram_post" } };
            } else {
        ...
        if (state.stage === CREATIVE_STAGES.PREVIEW && !inputConsumed) {
        ...
        return { response: { ok: false, text: "Processing...", mode: "instagram_post" } };
