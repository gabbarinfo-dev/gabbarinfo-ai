...
export async function resolveBusiness(session, metaRow, instruction, state) {
    ...
    try {
        ...
        // 4. Check if user instruction selects an account
        const lower = instruction.toLowerCase();
        const yesIntent = lower.includes("yes") || lower.includes("sure") || lower.includes("ok") || lower.includes("confirm") || lower.includes("go ahead");
        
        // A. Single account + Yes/Default
        if (validAccounts.length === 1 && (yesIntent || lower.includes("account") || lower.includes("use it"))) {
            const acc = validAccounts[0];
            return {
                complete: true,
                businessId: acc.instagram_business_account.id,
                businessName: acc.name,
                businessCategory: acc.category,
                logoUrl: acc.instagram_business_account.profile_picture_url
            };
        }

        // B. Name matching
        const matched = validAccounts.find(acc => lower.includes(acc.name.toLowerCase()));
        if (matched) {
            return {
                complete: true,
                businessId: matched.instagram_business_account.id,
                businessName: matched.name,
                businessCategory: matched.category,
