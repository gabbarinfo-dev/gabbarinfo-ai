 ...
 * @param {string} [params.targetInstagramId] - Explicit target Instagram Business Account ID
 * @param {string} [params.targetAccessToken] - Explicit access token for target account
 */
export async function executeInstagramPost({
  ...
  const cleanImageUrl = await ensureInstagramCompatibleJpeg({
    imageUrl,
    supabase: supabaseServer,
    bucket: "instagram-creatives",
    logger: (msg) => console.log(`[Instagram Organic] ${msg}`),
  });

  // 1. Fetch Meta Connection data from Supabase if not explicitly provided
  const cleanEmail = userEmail.toLowerCase().trim();
  let meta = null;
  if (!targetInstagramId || !targetAccessToken) {
    ...
    if (error || (!data && !targetInstagramId)) {
  ...
  // 2. Resolve Instagram ID (targetInstagramId > instagram_actor_id > ig_business_id)
  const instagramId = targetInstagramId || meta?.instagram_actor_id || meta?.ig_business_id;
  if (!instagramId) {
  ...
  // 3. Resolve Access Token (targetAccessToken > Supabase Token > System User Token fallback)
  const accessToken = targetAccessToken || meta?.fb_user_access_token || process.env.META_SYSTEM_USER_TOKEN;
  const isSystemToken = !targetAccessToken && !meta?.fb_user_access_token && !!process.env.META_SYSTEM_USER_TOKEN;

  if (accessToken) {
    console.log(`[Instagram Organic] Access Token Source: ${targetAccessToken ? "Explicit Param" : isSystemToken ? "Environment (META_SYSTEM_USER_TOKEN)" : "Supabase (fb_user_access_token)"}`);
    console.log(`[Instagram Organic] Token Preview: ${accessToken.substring(0, 10)}...`);
  }
  ...
  containerParams.append("image_url", cleanImageUrl);
  ...
  if (!containerRes.ok) {
    ...
    if (containerJson.error?.code === 10) {
      console.error("💡 [Meta Fix] Error #10 usually means the Instagram Page is not added as an asset to your System User in Business Manager.");
      console.error(`💡 [Meta Fix] ID being used: ${instagramId}. Check permissions for Token: ${accessToken.substring(0, 10)}...`);
    }
  ...
  // --- STEP 4: Reset Agent Memory (NEW) ---
  try {
    console.log(`[Instagram Organic] Resetting agent memory for: ${userEmail}`);
    ...
    console.warn("[Instagram Organic] Memory cleanup failed:", memErr.message);
  }
