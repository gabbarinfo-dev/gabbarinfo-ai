...
export async function executeInstagramPost({ userEmail, imageUrl, caption }) {
  ...
  // 3. Resolve Access Token (Supabase Token > System User Token fallback)
  const accessToken = meta.fb_user_access_token || process.env.META_SYSTEM_USER_TOKEN;
  const isSystemToken = !meta.fb_user_access_token && !!process.env.META_SYSTEM_USER_TOKEN;

  if (accessToken) {
    console.log(`[Instagram Organic] Access Token Source: ${isSystemToken ? "Environment (META_SYSTEM_USER_TOKEN)" : "Supabase (fb_user_access_token)"}`);
  ...
  // --- STEP 3: Cleanup Storage (NEW) ---
  if (imageUrl.includes("instagram-creatives")) {
    try {
      const fileName = imageUrl.split("/").pop().split("?")[0];
      console.log(`[Instagram Organic] Cleaning up storage for: ${fileName}`);
      await supabaseServer.storage.from("instagram-creatives").remove([fileName]);
    } catch (cleanupErr) {
      console.warn("[Instagram Organic] Auto-cleanup failed:", cleanupErr.message);
    }
  }
