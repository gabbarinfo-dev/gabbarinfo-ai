...
const GMB_REVIEWS_BASE = "https://mybusiness.googleapis.com/v4"; // confirmed working for reviews (v4)
...
/**
 * List reviews for a specific GMB location
 * @param {string} accessToken - valid OAuth access token
 * @param {string} locationName - full resource name e.g. "accounts/123/locations/456"
 * @param {number} pageSize - max reviews to fetch (default 10)
 */
export async function listGmbReviews({ accessToken, locationName, pageSize = 10 }) {
  try {
    const url = `${GMB_REVIEWS_BASE}/${locationName}/reviews?pageSize=${pageSize}&orderBy=updateTime%20desc`;
    const resp = await fetch(url, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${accessToken}`,
    ...
    const json = await resp.json();
    if (!resp.ok) {
      return { ok: false, status: resp.status, json, error: json.error?.message || "Failed to list reviews" };
    }
...
/**
 * Reply to (or update the reply of) a specific GMB review
 * @param {string} accessToken - valid OAuth access token
 * @param {string} reviewName - full resource name e.g. "accounts/123/locations/456/reviews/789"
 * @param {string} replyText - the reply body text
 */
export async function replyToGmbReview({ accessToken, reviewName, replyText }) {
  try {
    const url = `${GMB_REVIEWS_BASE}/${reviewName}/reply`;
    const resp = await fetch(url, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
    ...
    const json = await resp.json();
    if (!resp.ok) {
      return { ok: false, status: resp.status, json, error: json.error?.message || "Failed to reply to review" };
    }
...
/**
 * Delete a reply to a specific GMB review
 * @param {string} accessToken - valid OAuth access token  
 * @param {string} reviewName - full resource name e.g. "accounts/123/locations/456/reviews/789"
 */
export async function deleteGmbReviewReply({ accessToken, reviewName }) {
  try {
    const url = `${GMB_REVIEWS_BASE}/${reviewName}/reply`;
    const resp = await fetch(url, {
      method: "DELETE",
    ...
    return { ok: false, status: resp.status, error: json.error?.message || "Failed to delete reply" };
  } catch (err) {
    return { ok: false, status: 500, error: err.message };
  }
}
