...
export async function processMetaAdImage({
  ...
  try {
    ...
    // ── Bottom panel layout ────────────────────────────────────────────────
    // panelY = where the dark gradient starts
    const panelY = 680;
    let ty = panelY + 50;
    const parts = [];

    // ① Service name — large, white, centred
    const svcLines = wrapText(serviceStr, 56, 900);
    svcLines.forEach((l) => {
      parts.push(getPath(l, 512, ty, 56, "center", "#FFFFFF"));
      ty += 66;
    });
    ...
    if (taglineStr) {
      ty += 6;
      const tagLines = wrapText(taglineStr, 28, 860);
      tagLines.forEach((l) => {
    ...
    if (hasOffer) {
      ty += 8;
      const offerLines = wrapText(offer.toUpperCase(), 34, 880);
    ...
    console.log(
      "[ProcessMetaAdImage] Overlay complete. Size:",
      processed.length,
      "bytes"
    );
    return resultBase64;
